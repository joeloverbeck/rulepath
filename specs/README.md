...
| 10.1 | Gate 19.1 — Meldfall Ledger multi-round completion (ML-MATCH-006) | [`gate-19-1-meldfall-ledger-multi-round-completion.md`](gate-19-1-meldfall-ledger-multi-round-completion.md) | Planned | Authored 2026-06-26. Completes the Gate 19 exit item that shipped deferred: the deterministic Rust-owned multi-round transition (dealer rotation, round-only state reset, deterministic re-deal, cumulative play to the 500 target with tie continuation). Defect of record: `simulate` reports `completion_rate_percent=0.00` and the web board dead-ends at `round_settled`. No rule-text change; no ADR expected. |

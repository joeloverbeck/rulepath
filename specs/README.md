...
| 11.4 | Gate 20.4 — Starbridge Crossing in-match seat display names | [`gate-20-4-starbridge-crossing-in-match-seat-display-names.md`](gate-20-4-starbridge-crossing-in-match-seat-display-names.md) | Planned | Authored 2026-06-28 from a `/refine-game-ui` finding. In-match surfaces name seats by index (`Seat 1`) instead of display point name, against UI-INTERACTION §10B/§19; the shared `ModeControls` turn bar shows `Seat 1 to act` because Starbridge projects no Rust-owned per-seat display label the shell can consume. Spec: Rust projects the viewer-safe point label (catalog ring labels resolved per active seat) onto the public view (`ui.seat_labels`/`seats[].label`), the board drops its interim title-casing (commit `0d21913`), and `ModeControls` resolves via the existing shared path. Additive view projection; no determinism change. Independent of Gate 21. |

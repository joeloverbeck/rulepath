...
| — | Non-gate (River Ledger UX) — showdown legibility & table presentation | [`river-ledger-showdown-legibility-and-table-presentation.md`](river-ledger-showdown-legibility-and-table-presentation.md) | Planned | Non-blocking post-Gate-15 polish from `reports/river-ledger-correctness-and-presentation-report.md`. WS-A Rust-authored showdown explanation fields + panel redesign + worked-example e2e; WS-B neutral cards, hand-ranking reference, action/seat/turn copy, leak-safe teaching aid, no-casino audit; refresh `RULE-COVERAGE.md` UI rows. Correctness audit (Part A) found no defect. Independent of Gate 15.1. |

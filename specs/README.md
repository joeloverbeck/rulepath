...
| — | Non-gate (River Ledger UX) — showcase presentation V2 | [`river-ledger-showcase-ux.md`](river-ledger-showcase-ux.md) | Planned | Delta on the RIVLEDSHO pass, from `reports/river-ledger-showcase-ux-report.md`. Five workstreams: WS1 copy/leak guardrails (Rust seat display labels closing the `seat_N` authoring leak, per-action display rows, board-slot labels, seat-ledger copy, `Player N`→`Seat N`); WS2 `RiverLedgerShowdownPresentationV2` (ranked standings, decisive contrast-seat, hole/board card-usage marks, live region); WS3 central-board-well table recomposition + River-scoped tokens + scheduler-routed pacing; WS4 viewer-safe bot "why?"; WS5 original catalog icon. Re-audit (Part A) found no correctness defect. Non-blocking; independent of Gate 15.1. |

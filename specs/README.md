...
| — | Non-gate (web-shell replay) — import/export round-trip for full-length games | [`web-shell-replay-import-size-roundtrip.md`](web-shell-replay-import-size-roundtrip.md) | Planned | Authored 2026-06-28 by `/playtest-game starbridge_crossing`. Shared `ReplayImportExport` caps imports at 128 KB but the shell's own legitimate full-length export (Starbridge 6-seat, 2000-ply ≈ 549 KB) is rejected with `replay_too_large`, breaking the documented replay round-trip (SC-REPLAY-001, SC-VIS-004, SC-UI-002; FOUNDATIONS §9/§11). Presentation-only; no schema/hash change. Independent of Gate 21. Pending `/reassess-spec`. |

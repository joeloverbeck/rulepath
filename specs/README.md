...
| 11.3 | Gate 20.3 — Starbridge Crossing terminal outcome explanation | [`gate-20-3-starbridge-crossing-terminal-outcome-explanation.md`](gate-20-3-starbridge-crossing-terminal-outcome-explanation.md) | Planned | Spec written 2026-06-28. Closes the terminal outcome-explanation gap on shipped Gate 20: Rust must project a game-local `terminal_rationale` (decisive cause `finish_order_complete` / `turn_limit_progress_vector`, decisive rule IDs, seat-ring-ordered per-seat standings with the rank-1 winner flag — `SC-FINISH-004`/`SC-FINISH-006`), wasm-api must serialize it, and `StarbridgeCrossingBoard.tsx` — the only catalog board that never renders `OutcomeExplanationPanel` — must render it plus an `aria-live` mirror. Additive view field excluded from `stable_bytes`; no determinism artifact change. Independent of Gate 21. |

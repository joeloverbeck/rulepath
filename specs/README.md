...
| 10.2 | Gate 19.2 — Meldfall Ledger settlement-detail projection | [`gate-19-2-meldfall-ledger-settlement-detail-projection.md`](gate-19-2-meldfall-ledger-settlement-detail-projection.md) | Planned | Authored 2026-06-26 from web-app playtesting. Projects the already-public (`ML-VIS-006`) per-seat settlement breakdown (tabled-positive / in-hand-penalty / delta / cumulative / rank / winner) plus the round-end reason as a persistent `last_settlement` view field, so the web settlement panel can show *why* each delta is what it is and *how* the round ended. Presentation-completeness follow-on; no rule change; carrier pinned to the view to avoid a replay/hash ADR (with an explicit stop-and-ADR gate if that proves impossible). |

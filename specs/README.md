...
| 8C | Mechanical-scaffolding code extraction (Part C) | _(seed; unwritten)_ | Not started | **Blocked** on ADRs 0008/0009 `Accepted` + the 8M realignment `Done`. Report Part C / Batches 3–4: generic `EffectEnvelope` constructors, canonical seat-ID parse/format helpers, seat-count/ring arithmetic, versioned action-tree encoding + stable-byte writer, dev-only `game-test-support` crate, pairwise-no-leak + replay-profile harnesses, and bounded per-game retrofit waves. All hash/visibility migration governed by ADR 0009; no blanket golden regeneration. |
| 9 | Gate 18 — Spades (partnerships) | _(seed; unwritten)_ | Not started | Pending the **8M** realignment `Done` + **8C** scaffolding debt closed or explicitly excepted, ADRs 0008/0009 accepted, trace profiles + canonical seat grammar fixed, and the partnership/trick-taking atlas interlock resolved. Teams/partnership scoring + UI grouping stay game-local. |

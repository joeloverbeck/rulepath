# the first private licensed game Private Game Foundation-Readiness Change Plan
...
**Target private game:** *the first private licensed game*, a private licensed GMT COIN-series game, handled as private licensed IP.
...
### 2.3 the first private licensed game PDFs read
...
**Type:** amend + new-ADR
**Evidence:** `FOUNDATIONS.md` currently ranks later private stress tests below public IP-safe games and says private licensed experiments must be late, isolated, optional, non-public, and unable to shape public architecture. the first private licensed game milestone 1 is explicitly authorized to start now.
...
**Rationale:** Without this amendment, the first private licensed game violates the current priority order and the Gate-P tail. With it, the repo can learn from a monster game while refusing licensed-IP leakage or kernel capture.
...
**Type:** amend
**Evidence:** The roadmap currently keeps private monster-game work at the final Gate P tail after public Gates 21-23. the first private licensed game must begin now, parallel to that unfinished ladder.
...
**Rationale:** A parallel lane avoids pretending the first private licensed game is a public ladder gate while removing the doctrinal block on starting now.
...
**Type:** amend
**Evidence:** The contract currently defines “official/done” with Level-0 random-legal bot as the floor. the first private licensed game milestone 1 is a private hotseat/rule-complete target with no complex AI.
...
**Rationale:** The atlas needs vocabulary to review the first private licensed game without pushing private COIN nouns into public primitives.
...
**Type:** amend
**Evidence:** the first private licensed game will tempt authors to call card dispatch, faction eligibility routing, event-effect payloads, propaganda sequencing, and target filtering “scaffolding.” ADR 0008 allows only behavior-free scaffolding.
...
**Type:** amend
**Evidence:** The current architecture has one public workspace and a public `wasm-api` depending directly on every catalog game. the first private licensed game needs private crates, private renderer/catalog registration, and large progressive action trees for multi-space operations and event resolution.
...
**Type:** amend
**Evidence:** the first private licensed game has four fixed asymmetric factions, open negotiation, faction-specific victory, eligibility ordering, and no simple team partition.
...
**Type:** amend
**Evidence:** `rulepath_list_games` is the normal game catalog endpoint. The current public registry compiles public games into one public WASM crate. the first private licensed game must appear in normal lists only when the private repo/build is present.
...
**Type:** amend
**Evidence:** the first private licensed game needs a large map, many markers, four faction panels, eligibility/sequence display, event/propaganda context, action construction, and terminal/victory explanations. Public UI doctrine currently assumes public original visual targets.
...
**Type:** amend
**Evidence:** the first private licensed game milestone 1 needs four faction seats, all operation/special activity paths, all event-card branches, propaganda rounds, terminal detection, viewer-scoped replay/export, and private CI.
...
**Type:** amend
**Evidence:** `EVIDENCE-FIXTURE-CONTRACT.md` already has `private-source` visibility. the first private licensed game needs private card/source coverage, private fixtures, private trace exports, and public-safe aggregate evidence.
...
**Type:** clarify
**Evidence:** The current trace schema can hold command/action/effect/replay metadata. the first private licensed game does not inherently require a public trace schema change; it requires larger command/effect vocabularies and private-source profiles.
...
**Type:** amend
**Evidence:** the first private licensed game is too large for a single implementation task. The agent law already bans unbounded kernel changes, DSLs, and private IP in public files.
**Proposed change:** Add a private-monster protocol: first author/private-lane ADRs and spec placement; then private implementation specs split by setup/state, sequence/eligibility, operations, special activities, propaganda, events, visibility/replay, UI, CI; each task must name source/IP handling and public-boundary impact; no task may “implement the first private licensed game” as a single packet.
...
**Rationale:** Maintainers need a clean checklist without copying the first private licensed game content into public templates.
...
**Type:** amend
**Evidence:** the first private licensed game requires stable rule IDs for operations, special activities, card-driven sequence, propaganda rounds, and event effects, but public prose cannot copy licensed rules.
...
**Rationale:** A future the first private licensed game spec should not have to invent these categories ad hoc.
...
**Type:** amend
**Evidence:** The current source template bans proprietary text/assets and records source quality. the first private licensed game needs private official PDFs and possibly future strategy sources.
...
**Type:** amend
**Evidence:** Current how-to template has a single “players/seats/roles” and action-list path. the first private licensed game needs four faction-specific player guides, but public/private docs may not copy tutorial examples or player-aid prose.
...
**Type:** amend
**Evidence:** Current UI template already covers object counts, multi-seat layout, legal action mapping, previews, effects, outcome, accessibility, and no-leak. the first private licensed game needs much larger surfaces and private renderer/build proofs.
...
**Type:** amend
**Evidence:** Current template covers seat/opponent model, phases, threats, resources, inference, and kingmaking. the first private licensed game needs four asymmetric analyses and coalition/leader pressure.
...
**Type:** amend
**Evidence:** Future the first private licensed game AI needs four asymmetric policies, legal-action API use, coalition/kingmaking, and no flowchart-derived priorities.
...
**Type:** amend
**Evidence:** the first private licensed game’s action trees can be large because one operation can involve many spaces and a later event branch can touch many state subsystems.
...
**Type:** amend
**Evidence:** Current receipt includes completion profile, source/IP, trace profiles, viewer matrix, hidden-info matrix. the first private licensed game needs private CI and private build proof.
...
**Type:** amend
**Evidence:** The admission template asks whether implementation may begin under current foundations. the first private licensed game cannot begin until private-lane and event-card ADRs are proposed/accepted as required.
...
**Type:** amend
**Evidence:** the first private licensed game will exert pressure on graph/topology, event cards, resource accounting, action construction, and multi-faction victory. Private evidence should not silently trigger public third-use hard gates.
...
**Type:** new + amend
**Evidence:** Public checklist has a shipment rule and public artifact proof, but no private-release analogue. the first private licensed game needs private build readiness without public release.
...
**Type:** new-ADR
**Status:** Proposed
**Context:** `FOUNDATIONS.md` and ADR 0007 make private monster-game work a late tail after the public ladder. the first private licensed game is approved to start now as Rulepath’s first private licensed game.
...
**Consequences:** the first private licensed game milestone 1 can be admitted without waiting for Gate 23. ADR 0007 remains valid for the public ladder and for the original Gate P tail except where this ADR limits timing. Any private lane leak is a stop condition.
...
**Type:** new-ADR
**Status:** Proposed
**Context:** the first private licensed game’s event-card branches need conditional effects, target selection, persistent/temporary effects, and rule overrides. Existing doctrine forbids conditions/triggers/selectors/effects in static data and forbids DSLs without ADR.
...
**Target file(s):** [docs/ROADMAP.md](https://raw.githubusercontent.com/joeloverbeck/rulepath/142ddfae2be3ae2d7c861ab65f2c786a49de54ac/docs/ROADMAP.md)
**Type:** amend
**Evidence:** D5 requires a the first private licensed game milestone-1 lane/gate.
...
**Type:** amend
**Evidence:** `specs/README.md` currently tracks public gates and Gate P as not started. D5 requires private the first private licensed game milestone placement and private spec fields, but public specs must not leak licensed content.

...
| fixed 2D occupancy | `three_marks`, `column_four`, `directional_flip` | `rejected/deferred with rationale` | Gate 6 as-built outcome confirms the ledger decision: keep local for now. The broad occupancy shape repeats, but a useful helper boundary is still too mixed across static 3 by 3 cells, bottom-origin gravity columns, and top-origin 8 by 8 flip rays; no `engine-core` or `game-stdlib` promotion. See `games/directional_flip/docs/PRIMITIVE-PRESSURE-LEDGER.md` and `games/directional_flip/docs/MECHANICS.md`. | Reopen at the next official spatial game, or if a post-Gate 6 audit proves one narrow behavior-free helper without trace/hash migration. |
...
| coordinate/targeted placement | `three_marks`, `column_four`, `directional_flip` | `rejected/deferred with rationale` | Gate 6 as-built outcome confirms the ledger decision: direct cells, column-derived landing cells, and direct cells plus forced pass are not one placement primitive. Stable coordinate parsing/formatting remains game-local for Gate 6. See `games/directional_flip/docs/PRIMITIVE-PRESSURE-LEDGER.md` and `games/directional_flip/docs/MECHANICS.md`. | Reopen if another repeated direct-cell game proves one stable coordinate helper without origin/order flags. |

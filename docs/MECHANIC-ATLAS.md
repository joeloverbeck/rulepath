...
| fixed 2D occupancy | `three_marks`, `column_four`, `directional_flip` | `rejected/deferred with rationale` | Gate 6 ledger decision: keep local for now. The broad occupancy shape repeats, but a useful helper boundary is still too mixed across static 3 by 3 cells, bottom-origin gravity columns, and top-origin 8 by 8 flip rays; no `engine-core` or `game-stdlib` promotion. See `games/directional_flip/docs/PRIMITIVE-PRESSURE-LEDGER.md`. | Reopen after Directional Flip is implemented if local duplicated coordinate code proves a narrow behavior-free helper, or at the next official spatial game. |
| simple line/pattern detection | `three_marks`, `column_four`, `directional_flip` | `rejected/deferred with rationale` | Gate 6 ledger decision: keep line/ray/flip scanning local. Static Three Marks lines, Column Four four-direction terminal scans, and Directional Flip eight-direction bracketed flips differ enough that shared pattern detection would risk policy in a helper. See `games/directional_flip/docs/PRIMITIVE-PRESSURE-LEDGER.md`. | Reopen only for behavior-free ray stepping after as-built evidence; do not promote win/flip/capture legality. |
| coordinate/targeted placement | `three_marks`, `column_four`, `directional_flip` | `rejected/deferred with rationale` | Gate 6 ledger decision: direct cells, column-derived landing cells, and direct cells plus forced pass are not one placement primitive. Stable coordinate parsing/formatting remains game-local for Gate 6. See `games/directional_flip/docs/PRIMITIVE-PRESSURE-LEDGER.md`. | Reopen if repeated direct-cell games prove one stable coordinate helper without origin/order flags. |
...
| terminal line highlighting | `three_marks`, `column_four`, `directional_flip` | `rejected/deferred with rationale` | Gate 6 ledger decision keeps terminal and flip highlighting/effect geometry game-local. Three Marks and Column Four highlight terminal lines; Directional Flip highlights Rust-provided preview/apply flip sets and grouped effects, which is related presentation pressure but not one shared primitive. See `games/directional_flip/docs/PRIMITIVE-PRESSURE-LEDGER.md`. | Reopen only if a later UI/effect spec proves a behavior-free shared highlight projection contract. |
| directional scanning and grouped flips | `directional_flip` | `rejected/deferred with rationale` | Gate 6 ledger decision: implement Directional Flip rays, legal bracketing, previews, grouped flips, and effect ordering locally. The helper question is resolved as defer/reject for Gate 6, with no helper promotion before local benchmark/replay evidence exists. See `games/directional_flip/docs/PRIMITIVE-PRESSURE-LEDGER.md`. | Stage 4 exit must confirm local implementation, traces, and docs match the ledger decision. |

...
| River Ledger / Gate 15 Hold'Em-family bundle | `high_card_duel`, `poker_lite`, `plain_tricks`, `masked_claims`, `token_bazaar`, `event_frontier`, Infra A-D N-seat surfaces, `river_ledger` | `rejected/deferred with rationale` | Gate 15 records the final pressure review in [../games/river_ledger/docs/PRIMITIVE-PRESSURE-LEDGER.md](../games/river_ledger/docs/PRIMITIVE-PRESSURE-LEDGER.md): standard deck, hidden hole cards, staged community reveal, N-seat projections, fixed-limit contribution ledger, seven-card evaluator, showdown explanation, split allocation, and remainder order stay game-local. The comparison set shows real repeated pressure, but the shared subset is either trivial shuffle/deal or too behavior-bearing across visibility, betting, evaluator, allocation, bots, replay, and UI. Decision: `game-local / no promotion`; no `game-stdlib` helper, no `engine-core` noun, no trace/hash migration, and no §10A promotion debt. | Reopen for Gate 15.1 side-pot/all-in work or before another official game repeats a close N-seat hidden-information betting/evaluator/allocation bundle. |

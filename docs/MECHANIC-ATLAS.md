...
| numeric trick bid / contract-vs-result / last-bidder hook | `vow_tide` | `local-only` first official use | Gate 17 records the first official numeric trick-contract mechanic in [../games/vow_tide/docs/PRIMITIVE-PRESSURE-LEDGER.md](../games/vow_tide/docs/PRIMITIVE-PRESSURE-LEDGER.md): Rust generates sequential public `bid/<n>` leaves, applies the dealer-only `H-S` hook, validates range/order/immutability, and emits public bid effects. No shared bid/contract/scoring framework is authorized; legality and future scoring remain typed game-local Rust, not static data or TypeScript. | Reopen at Gate 18 or the next close numeric contract game before any second-use comparison or shared helper proposal. |
...
Current debt: _None_. Last reviewed during Gate 17 Vow Tide closeout on
2026-06-21; the `game-stdlib::trick_taking` helper promotion conformed Plain
Tricks and Briar Circuit in-gate, Vow Tide uses the helper, and no open

...
| Hearts, Oh Hell, and Spades | follow-suit, trick resolution, trick-winner turn order, hidden-hand no-leak, partnerships | Gate 16 Briar Circuit records the second-use trick-taking comparison and keeps it local/deferred. Reopen trick-taking before Gate 17 Oh Hell as the third close use. Partnership/team scoring needs its own comparison rather than folding into seat IDs. |
...
| follow-suit legality | `plain_tricks`, `briar_circuit` | `repeated-shape candidate`; second-use comparison recorded | Gate 10.1 records first official follow-suit pressure: leader may play any held card; follower must play led suit if holding it, otherwise any held card. Gate 16 records the second-use comparison in [../games/briar_circuit/docs/PRIMITIVE-PRESSURE-LEDGER.md](../games/briar_circuit/docs/PRIMITIVE-PRESSURE-LEDGER.md): Briar Circuit repeats the core shape but adds four seats, 2 clubs opening, first-trick point restrictions, hearts-broken lead restrictions, pass privacy, and penalty scoring. Decision: keep local/defer extraction; no helper promotion and no §10A debt. | Gate 17 Oh Hell is the third-use hard-gate trigger. |
| trick resolution / led-suit comparator | `plain_tricks`, `briar_circuit` | `repeated-shape candidate`; second-use comparison recorded | Gate 10.1 records first official trick-resolution pressure: led suit is established by the leader; same-suit higher rank wins; off-suit follower cards never win. Gate 16 records the second comparison: Briar Circuit repeats led-suit comparison over four-card tricks but keeps card/rank/suit types, point-card policy, moon scoring, and outcome semantics local. No `engine-core` card, suit, rank, or trick noun and no `game-stdlib` helper are authorized. | Gate 17 Oh Hell is the third-use hard-gate trigger. |
| trick-winner-leads turn order | `plain_tricks`, `briar_circuit` | `repeated-shape candidate`; second-use comparison recorded | Gate 10.1 records first official trick-winner-led turn order: a resolved trick's winner becomes next trick leader unless the round closes. Gate 16 records the second comparison: Briar Circuit repeats winner-leads sequencing across 13-trick hands, but its pass cycle, first lead, scoring, threshold, and no-leak obligations remain behavior-bearing local policy. | Gate 17 Oh Hell is the third-use hard-gate trigger. |
| deal rotation / trick-round redeal | `plain_tricks`, `briar_circuit` | `repeated-shape candidate`; second-use comparison recorded | Gate 10.1 records first official trick-round deal rotation: round 1 starts with `seat_0`, round 2 redeals from the continuing RNG stream and starts with `seat_1`. Gate 16 records the second comparison: Briar Circuit deals a full 52-card deck, rotates dealer clockwise after each hand, and layers pass-cycle rotation over deterministic deal. This is not a generic deal/redeal primitive. | Gate 17 Oh Hell is the third-use hard-gate trigger. |

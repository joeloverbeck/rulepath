...
| public resource accounting / visible market costs | `token_bazaar` | `local-only` | Gate 9 records first official public resource/accounting pressure: public supply, public inventories, exact payments, exchange supply returns, visible contract costs, deterministic slot refill, score accounting, and public tie-breaks. No `game-stdlib` promotion is authorized; see `games/token_bazaar/docs/MECHANICS.md`. | Compare after a second economy/betting game; third repeated use requires a primitive-pressure ledger decision before extraction. |
...
| resource accounting | `token_bazaar`; later betting/economy games are comparison candidates | `local-only` initially; repeated-shape candidate after second economy/betting use | Gate 9 keeps resource/accounting local as first official use. No formulas in data; compare ledgers before third use. | Stage 9/10 review. |

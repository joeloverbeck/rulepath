...
|---|---|---|---|---|
| deterministic shuffle / private hand / staged reveal | `high_card_duel`, `poker_lite` | `repeated-shape candidate` | Gate 8 records first official local card/deck pressure: deterministic shuffle, private hands, hidden face-down commitment, simultaneous reveal, viewer-filtered effects, and no-leak public replay export. Gate 10 adds Crest Ledger as the second similar official use with deterministic shuffle, owner-private crests, hidden center crest, staged center reveal, grouped showdown reveal, yield without private reveal, and browser no-leak export evidence. Keep local; no `game-stdlib` promotion is authorized. `blackjack_lite` remains a deferred comparison case only under [ADR 0006](adr/0006-blackjack-lite-roadmap-placement.md). | Third card/private-hand use, likely `plain_tricks` if it repeats the shape, must record a ledger decision before extraction. |
| public resource accounting / shared ledgers | `token_bazaar`, `poker_lite` | `repeated-shape candidate` | Gate 9 keeps Token Bazaar resource/accounting local as first official use. Gate 10 adds Crest Ledger as second similar pressure with public contributions, a shared pool, exact pledge additions, yield award, showdown allocation, and split handling. The shapes differ enough to keep local and compare; no formulas in data and no helper promotion. | Third economy/accounting use requires a primitive-pressure ledger decision before extraction. |

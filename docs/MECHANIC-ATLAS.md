...
| Hearts, Oh Hell, and Spades | follow-suit, trick resolution, trick-winner turn order, hidden-hand no-leak, partnerships | Gate 17 Vow Tide resolves the third close follow-suit/comparator use by promoting the narrow pure `game-stdlib::trick_taking` helper. Gate 18 Blackglass Pact reuses those helpers unchanged with permanent spades trump, while winner-leads, dealing, dealer rotation, bidding, scoring, hidden-hand projection, and all partnership/team behavior remain game-local. Gate 18 must not fold partnerships into generic seat identity. |
...
| follow-suit legality | `plain_tricks`, `briar_circuit`, `vow_tide`, `blackglass_pact` | `promoted primitive` for `game-stdlib::trick_taking::follow_suit_indices` | Gate 17 resolves the third-use hard gate by promoting one pure helper that returns led-suit held-item indices in stable input order, or every index when void. The helper inspects only caller-projected suit keys and returns indices; ownership, legal leads, first-trick exceptions, hearts-broken restrictions, diagnostics, effects, visibility, and action-tree shape remain game-local. Tickets `GAT17VOWTIDOHHEL-003` and `GAT17VOWTIDOHHEL-004` completed same-gate Plain Tricks/Briar Circuit conformance, Vow Tide uses the helper for its trick behavior, and replay/fixture/hash evidence stayed green. Gate 18 Blackglass Pact reuses the helper unchanged after game-local actor, owner, and broken-spades checks. | None after Gate 18 conformance closeout; future games may reuse the helper for matching follow-suit selection but must keep partnership/scoring policy local. |
| trick resolution / led-suit comparator | `plain_tricks`, `briar_circuit`, `vow_tide`, `blackglass_pact` | `promoted primitive` for `game-stdlib::trick_taking::winning_play_index` | Gate 17 resolves the third-use hard gate by promoting one pure winning-index helper: highest caller-projected trump when any trump is played, otherwise highest caller-projected led suit, with stable first occurrence on equal ranks and `None` for empty/no-eligible input. The helper defines no card, suit, rank, trick, seat, trump-source, score, effect, or view type. Plain Tricks and Briar Circuit use it with `trump = None`; Vow Tide uses a public per-hand trump; Blackglass Pact uses `Some(Spades)` for permanent trump and maps the winning index to seat/team locally. | None after Gate 18 conformance closeout; future trump/partnership games may reuse the pure comparator only when their caller-owned policy fits. |
| trick-winner-leads turn order | `plain_tricks`, `briar_circuit`, `vow_tide` | `rejected/deferred with rationale`; reviewed as helper anti-example | Gate 17 explicitly reviews winner-leads as related third-use pressure but rejects promotion because changing active leader requires game-local phase, hand-close, score, effect, replay, and visibility policy. `game-stdlib::trick_taking` returns only a winning play index; callers map that index to a seat and mutate state locally. | Reopen only if later official games prove a behavior-free turn-order helper that does not encode phase/scoring/effects policy. |
| deal rotation / trick-round redeal | `plain_tricks`, `briar_circuit`, `vow_tide` | `rejected/deferred with rationale`; reviewed as helper anti-example | Gate 17 explicitly reviews deal/redeal/dealer rotation as related third-use pressure but rejects promotion. Plain Tricks uses a short two-round continuing-RNG deal, Briar Circuit uses full-deck Hearts hands with pass-cycle rotation, and Vow Tide uses variable hand sizes plus turn-up trump and hidden stock. RNG partitioning, schedule, dealer, first leader/bidder, stock, effects, traces, and no-leak policy remain game-local. | Reopen only if a later official game repeats a close deterministic deal/schedule shape and a narrow behavior-free helper can preserve traces and visibility. |
| numeric trick bid / contract-vs-result / last-bidder hook | `vow_tide`, `blackglass_pact` | `local-only` second-use keep-local | Gate 17 records the first official numeric trick-contract mechanic in [../games/vow_tide/docs/PRIMITIVE-PRESSURE-LEDGER.md](../games/vow_tide/docs/PRIMITIVE-PRESSURE-LEDGER.md): Rust generates sequential public `bid/<n>` leaves, applies the dealer-only `H-S` hook, validates range/order/immutability, and emits public bid effects. Gate 18 records the second-use comparison in [../games/blackglass_pact/docs/PRIMITIVE-PRESSURE-LEDGER.md](../games/blackglass_pact/docs/PRIMITIVE-PRESSURE-LEDGER.md): Blackglass Pact has nil and blind nil, no dealer hook, team aggregation, bags, persistent team scores, and partner no-leak. No shared bid/contract/scoring framework is authorized; legality and scoring remain typed game-local Rust, not static data or TypeScript. | Reopen before a third close numeric trick-contract game. |
| fixed competitive partnership / team scoring | `blackglass_pact` | `local-only` first official use | Gate 18 records the first fixed competitive partnership/team scoring shape in [../games/blackglass_pact/docs/PRIMITIVE-PRESSURE-LEDGER.md](../games/blackglass_pact/docs/PRIMITIVE-PRESSURE-LEDGER.md): stable North-South and East-West teams, team contract aggregation, individual nil/blind nil inside team scoring, persistent bags, team standings/outcomes, and explicit partner no-leak. This is behavior, not seat identity or behavior-free scaffolding. | Reopen when a second official competitive partnership/team scoring game appears; hard-gate before any third close team-helper proposal. |
| meld validation: sets and runs | `meldfall_ledger` | `local-only` first official use | Gate 19 records first official rummy-family meld validation in [../games/meldfall_ledger/docs/PRIMITIVE-PRESSURE-LEDGER.md](../games/meldfall_ledger/docs/PRIMITIVE-PRESSURE-LEDGER.md): same-rank sets, same-suit runs, ace-low/high/no-wrap policy, ownership checks, diagnostics, effects, bots, and UI remain in `games/meldfall_ledger`. No `game-stdlib` rummy helper, no `engine-core` rank/suit/meld noun, and no static-data rule predicate are authorized. | Record comparison on a second official meld game; hard-gate before a third close meld/tableau helper. |
| public meld tableau and score-credit zone model | `meldfall_ledger` | `local-only` first official use | Gate 19 records first public rummy tableau pressure in [../games/meldfall_ledger/docs/PRIMITIVE-PRESSURE-LEDGER.md](../games/meldfall_ledger/docs/PRIMITIVE-PRESSURE-LEDGER.md): public meld ids, origin seat, ordered cards, and per-card played-by credit are game behavior tied to lay-off and scoring. Public transport is not mechanical scaffolding. | Reopen before a third close public meld/tableau/zone use. |
| draw/discard zones with multi-card pickup | `meldfall_ledger` | `local-only` first official use | Gate 19 records first discard-tail pickup pressure in [../games/meldfall_ledger/docs/PRIMITIVE-PRESSURE-LEDGER.md](../games/meldfall_ledger/docs/PRIMITIVE-PRESSURE-LEDGER.md): stock count, public discard order, selected discard plus newer-card pickup, and immediate-use commitment stay Rust-owned and game-local. | Reopen before a third close draw/discard-zone helper. |
| lay-off onto any public meld | `meldfall_ledger` | `local-only` first official use | Gate 19 records first lay-off/tableau-extension pressure in [../games/meldfall_ledger/docs/PRIMITIVE-PRESSURE-LEDGER.md](../games/meldfall_ledger/docs/PRIMITIVE-PRESSURE-LEDGER.md): extension legality, target meld shape, no rearrangement, and score-credit attribution remain local behavior. | Reopen before a third close lay-off/tableau-extension use. |
| multi-round rummy cumulative scoring to target | `meldfall_ledger` | `local-only` first official use | Gate 19 records first rummy-style cumulative scoring pressure in [../games/meldfall_ledger/docs/PRIMITIVE-PRESSURE-LEDGER.md](../games/meldfall_ledger/docs/PRIMITIVE-PRESSURE-LEDGER.md): tabled-card positives, in-hand penalties, negative scores, score-credit owner, target 500, and tie continuation remain typed Rust behavior. | Reopen before a third close rummy-style cumulative scoring target. |
...
| River Ledger / Gate 15 Hold'Em-family bundle and Gate 15.1 side-pot/all-in extension | `high_card_duel`, `poker_lite`, `plain_tricks`, `masked_claims`, `token_bazaar`, `event_frontier`, Infra A-D N-seat surfaces, `river_ledger` | `rejected/deferred with rationale` | Gate 15 records the final pressure review in [../games/river_ledger/docs/PRIMITIVE-PRESSURE-LEDGER.md](../games/river_ledger/docs/PRIMITIVE-PRESSURE-LEDGER.md): standard deck, hidden hole cards, staged community reveal, N-seat projections, fixed-limit contribution ledger, seven-card evaluator, showdown explanation, split allocation, and remainder order stay game-local. Gate 15.1 reopens the side-pot/all-in pressure in the same ledger and again defer/rejects promotion: finite stacks, full-unit reopen state, contribution-layer construction, folded-money inclusion, uncalled returns, per-pot eligibility, independent split/remainder allocation, and viewer-safe allocation explanation are implemented locally because the full nested-eligibility shape has one real implementation pressure point and remains too behavior-bearing across visibility, betting, evaluator, allocation, bots, replay, and UI. Decision: reuse River Ledger's local `winners_in_button_order`, implement the layer constructor/allocation locally, no `game-stdlib` helper, no `engine-core` noun, no global trace/hash migration, and no §10A promotion debt. | Reopen before another official game repeats a close N-seat hidden-information betting/evaluator/allocation bundle or when repeated defects prove a narrow behavior-free helper can cover the real shared surface without policy flags. |

Gate 15.1 closeout evidence for the row above is recorded in
[../games/river_ledger/docs/RULES.md](../games/river_ledger/docs/RULES.md),
[../games/river_ledger/docs/RULE-COVERAGE.md](../games/river_ledger/docs/RULE-COVERAGE.md),
and [../games/river_ledger/docs/BENCHMARKS.md](../games/river_ledger/docs/BENCHMARKS.md).

## 10A. Open promotion-debt register

Current debt: _None_. Last reviewed during Gate 20 Starbridge Crossing
...
| graph/topology/path-jump legality | `frontier_control`, `event_frontier`, `starbridge_crossing`; Gate 21 Pachisi-family race is the next comparison candidate | `rejected/deferred with rationale` | Gate 20 records the third related topology/path hard-gate decision in [../games/starbridge_crossing/docs/PRIMITIVE-PRESSURE-LEDGER.md](../games/starbridge_crossing/docs/PRIMITIVE-PRESSURE-LEDGER.md): Starbridge Crossing's masked 121-space star, six-direction neighbor metadata, home/target triangles, occupied-midpoint hop checks, empty landing checks, stop-anywhere jump chains, repeated-landing guard, finish ranks, and progress-vector turn-limit fallback are related to prior graph pressure but not the same narrow helper shape. `game-stdlib::board_space` is audited not applicable because its promoted scope is rectangular dimensions, coordinates, row-major iteration, offsets, `rNcM` identity, and parity. Decision: defer/reject promotion; no graph/topology/path/jump helper, no `engine-core` noun, no prior-game conformance, and no §10A promotion debt. | Gate 21 must compare its track topology, deterministic chance, capture, and safety semantics against `board_space`, Frontier Control, Event Frontier, and Starbridge Crossing before any helper proposal. |

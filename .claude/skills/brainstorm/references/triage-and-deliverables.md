...
- **Each `O<N>` carries a disposition.** Since the closed verdict buckets are scoped to source-report items and omitted here, label each finding with a disposition from this fixed set: `fix-now` (warrants action now), `flag-for-follow-up` (sound but routed to a later deliverable — name the follow-up shape + re-evaluation trigger), or `informational` (context, no action). Don't invent per-session labels. (The action-shaped sub-case below supersedes this: when the whole diagnostic resolves to one recommended *action*, use Step 3's recommendation shape instead of per-finding dispositions.)

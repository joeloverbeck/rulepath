...
| Architectural decision, or amend/supersede `docs/FOUNDATIONS.md` (constitutional change) | `docs/adr/ADR-NNN-<slug>.md` from `docs/adr/ADR-TEMPLATE.md` — flag a constitution supersession explicitly, require sign-off |
...
Adjust options to the deliverable: for an ADR, offer decompose-into-agent-tasks / implement / done; for agent tasks, offer begin-first-task / refine / done. Use `AskUserQuestion` when its schema is available; inline numbered options are an acceptable fallback. If the user picks an option that invokes another skill, invoke it. If they pick "done," end.

...
2. **Crate skeleton + workspace wiring** — creates `games/<game>/` (all-`(new)`; see §Greenfield foundation batch, new-crate clause).
3. **Pipeline tickets** by `OFFICIAL-GAME-CONTRACT` sub-concern: rules core → public view/visibility → effects → replay → bots → tests → golden traces/fixtures → benchmarks → WASM exposure → web renderer → web smoke.
4. **Bot-strategy docs before the Level 2 bot** (`COMPETENT-PLAYER.md`, `BOT-STRATEGY-EVIDENCE-PACK.md`) when the gate ships a Level 2 policy.
5. **Tool-validated docs with their validators** (see §Cross-cutting docs ticket → Tool-validated docs): `RULE-COVERAGE.md` with the rule-coverage registration; `BENCHMARKS.md` with the benches.
6. **Tool + CI registration** — per-tool match arms (`simulate` / `replay-check` / `fixture-check` / `rule-coverage`), then the CI workflow steps.
7. **Trailing game docs** (MECHANICS / UI / AI / ADMISSION / PUBLIC-RELEASE-CHECKLIST) + a **capstone** (mechanic atlas, status hygiene, spec/index `Done`-flip). §Exit-criteria acceptance is typically **distributed** across the bench / wasm-smoke / tool / browser-smoke / CI tickets rather than forced into a synthetic capstone.

...
5. `<namespace>` is provided, OR derive one from the gate number and abbreviated slug (e.g. `specs/gate-0-repository-skeleton.md` → `GATE0SKEL`) and ask the user to confirm or override before Step 1. **Whether provided or derived, verify the namespace does not collide with an existing series**: `ls tickets/` and `archive/tickets/` (and `git log` when those hold only `README.md`/`_TEMPLATE.md`) for `<NS>-NNN`. On any match, stop and ask the user to confirm continuing that series or pick a distinct namespace — a *provided* namespace colliding with a **closed/archived** series is the common miss (it misattributes new work to finished history; keep consistent with `references/decomposition-patterns.md` §"Spec-supplied candidate ticket IDs" and brainstorm's `triage-and-deliverables.md` §"ID assignment"). **When the provided namespace differs from the spec's own `Ticket prefix:` header field** (e.g. the spec declares `RIVLEDFIX` but the run namespace is `RIVLEDSHOSEA`), note the override and the prefix→namespace mapping in the Step-4 table — the spec's internal `PREFIX-NNN` references (its §Ticket-decomposition candidate IDs) will not match the produced filenames, so a reviewer needs the 1:1 map.
...
**Load `references/codebase-validation.md`.** Validate the spec's assumptions against the current codebase, surface Issues, and obtain a per-Issue disposition before Step 3. A spec that was reassessed via `/reassess-spec` earlier this session with all findings resolved qualifies for the abbreviated spot-check path documented in the reference — which still requires loading `references/codebase-validation.md` and rendering its `(a)–(g)` spot-check inline list (it is a targeted 3–5 grep pass, not a skip of the reference or its output).

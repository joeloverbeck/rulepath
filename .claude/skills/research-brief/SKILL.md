...
1. **Write the brief** to `reports/<topic>-research-brief.md`, following the canonical anatomy in `references/brief-template.md`. `<topic>` is a short kebab-case slug of the target. If a brief on the same subject already exists in `reports/`, do **not** reuse its slug — choose a distinct slug that names this brief's new angle (e.g. `river-ledger-showcase-ux` alongside an existing `river-ledger-correctness-ux`), and reference the prior brief as the delta baseline in §1/§2. `Write` overwrites without warning, so a reused slug silently destroys the earlier brief; guard the brief file the same way Step 6.2 guards manifests from same-name overwrite.

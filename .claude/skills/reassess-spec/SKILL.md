...
- `docs/FOUNDATIONS.md` — the constitution. Skip only if read earlier this session and unmodified. **If the spec's own repo has no `docs/FOUNDATIONS.md`, the spec is a cross-repo / pinned-sibling spec — read FOUNDATIONS (and any ADR / kernel references) from the pinned sibling repo; see Worktree, Cross-Repo & Plan-Mode Awareness.**
...
## Worktree, Cross-Repo & Plan-Mode Awareness
...
**Cross-repo / pinned-sibling specs.** When the spec lives in a repo that *depends on* a pinned sibling (e.g. a sanctioned private-game lane whose `*-public-pin.toml` pins the public governing repo), the spec's own repo will not contain `docs/FOUNDATIONS.md`, the ADRs, or the kernel crates — they live in the pinned sibling. In that case: (1) locate the pin file and resolve the sibling repo path; (2) read FOUNDATIONS / ADRs / `engine-core` references **from the sibling**, not the spec's repo; (3) **split Step-3 validation by repo** — private-crate references validate against the spec's repo, kernel/contract references against the sibling **at the pinned commit**; (4) verify the spec's stated provenance commit against the pin — a mismatch is a finding (note its validation impact, e.g. confirm the cited kernel files are unchanged between the two commits, which downgrades it to a stale-statement note); (5) verify the spec's **own-repo** provenance commit against that repo's current HEAD (`git -C <spec-repo> rev-parse HEAD`) — the pin check covers the *sibling*, not the spec's own crate, so an advanced own-repo (new commits since the stated provenance) stales the cited `file:line` numbers even though every symbol still resolves; when it advanced, treat every line citation as potentially stale and re-validate each symbol at current HEAD (drift is a finding — refresh the numbers or re-anchor to grep-stable symbols per 3.3). This is a *legitimate dependency*, distinct from the externally-generated *foreign-repository scaffolding* of the Pre-Process note — do **not** strip a pinned cross-repo reference as scaffolding. **Validation mechanism (cross-repo).** Prefer direct operator-controlled batched greps *split by repo* over Explore-agent delegation regardless of reference count — repo-routing (which grep resolves against the spec's repo vs. the pinned sibling) must be controlled, and a delegated agent may resolve paths against the wrong tree; the Step-2 reference-count checkpoint's themed-Explore steer is for single-repo counts. **Prior-reassessment stamp.** When the spec's provenance already records a *prior* reassessment at a specific commit (a re-reassessment across sessions — the same-session Re-reassessment shortcut does not apply), that recorded HEAD **and** its `+N commits` delta are themselves refresh targets: verify the recorded commit against current HEAD, refresh the SHA, recompute the delta, and confirm the substantive claim (which files unchanged, which symbols still resolve) still holds at current HEAD. **Own-repo index naming.** The spec's own-repo progress index may be named differently from `specs/README.md` (e.g. a `*-implementation-index.md` with its own status tracker) — discover it rather than assuming the public name, and validate both its sequencing / status-tracker role (3.4) and any design-plan / claim-source role it plays when the spec cites it as its source (3.10). A single such index file can serve both roles at once.
...
**Post-gate correctness/contract follow-on (no ROADMAP gate)**: a spec that fixes a defect or contract gap on an already-shipped gate — Header reads "Gate X.Y … fix on shipped Gate X" / "post-`Done`", and no `docs/ROADMAP.md` gate backs it (the non-gate / post-gate-presentation spec sub-type `brainstorm` recognizes) — still classifies (a)/(b)/(c) by its deliverables (commonly (b)). But the ROADMAP-anchored validations have no referent: 3.10's "every ROADMAP Exit/Not-allowed line must map" and 3.11's "exit criteria row-for-row to the ROADMAP gate's exit list" are replaced by validating that §Exit-criteria / §Forbidden-changes are internally grounded in the spec's own Reproduction / Objective and the cited contract docs. Do NOT fire a HIGH Issue for the absence of a ROADMAP referent; do still validate that the self-originating exit criteria are present, concrete, and re-runnable.

**Private-lane milestone spec (no ROADMAP gate)**: a *forward-feature* spec in a sanctioned private lane anchored to a private completion-profile milestone (e.g. `private-milestone-1-rule-complete`, with a public tracker anchor like `P1-M1`) rather than a public `docs/ROADMAP.md` gate gets the same treatment as the post-gate follow-on above — classify (a)/(b)/(c) by its deliverables, do **not** fire the unmapped-ROADMAP-line Issue for the absent public gate, and validate §Exit-criteria as self-originating from the spec's own Objective and the cited contract docs. Distinguish it from the post-gate follow-on: this is *new* forward work in the private lane, not a defect/contract fix on a shipped gate.
...
**Externally-generated / commit-pinned specs**: when the spec carries generation-harness scaffolding — a commit-pinned evidence ledger (lists of raw source URLs at a fixed SHA), "exact-commit workflow" / fetch-discipline boilerplate, contamination checks, or references to a foreign repository — treat stripping that scaffolding as an expected HIGH Issue at Step 5, and route any genuine provenance into a normal source-reference section. This is orthogonal to the (a)–(d) class: tag it alongside whichever class the spec's deliverables fall under. Such specs also commonly substitute commit-pin header fields (`Target commit` / `Target repository`) for a canonical Header and carry their own `§`-numbering, so the reform-depth question (Steps 5-6, `references/findings-and-questions.md`) routinely rides alongside the scaffolding strip — consider both together rather than as separate discoveries. When the scaffolding lives in a *cited source document* rather than the spec itself (the spec references a report that carries the commit-pinned ledger / URL list, but the spec carries only traceability anchors and a single commit-pin note), do **not** strip — treat the source's IDs as traceability anchors and validate its claims under 3.10; the strip applies only to scaffolding the spec itself carries.
...
- **Spec sequencing** — the `authority order` list and the Sequencing section's predecessor/successor gate; the row for this spec in the `specs/README.md` index (or, for a cross-repo / private-lane spec, the own-repo index discovered per the Cross-repo section, which may carry a different filename).
- **Source documents** — when the spec cites an external source (a `ROADMAP.md` gate, a brainstorm output under `docs/plans/`, a report) in its Objective / Scope / Work breakdown, extract the path AND enumerate its actionable claims at Step 2 itself (read the source; for oversized docs use targeted greps with permissive anchoring). Tag each claim's adjudication status (accept / reject / defer / unadjudicated) by scanning the spec's Scope / Deliverables / Out of scope. This feeds Step 3.10. **Not source documents**: a precedent sibling-spec the spec merely *mirrors* (validate it under 3.4/3.7 sibling-gate refs, not 3.10); standards/UX citation footnotes (NNG/WCAG/etc. — provenance, not adjudicable claims); and authority-order contract/architecture docs (`docs/ARCHITECTURE.md`, `docs/*-CONTRACT.md`, `docs/ENGINE-GAME-DATA-BOUNDARY.md`) the spec cites as its governing contracts — validate these under 3.4/3.8 as contracts whose fidelity the spec must preserve, not as 3.10 claim-bearing sources to adjudicate. When only these are present, the source-document engagement checkpoint reads `N/A — no external source cited`.
...
- **No scope creep**: the deliverable is the updated spec file. Do not write AGENT-TASKs, create tickets, start implementation, or edit sibling spec files — unless a Step 6 question response explicitly authorizes a named sibling-spec edit (record it in Step 8 under a "Cross-spec scope extension" line). Updating this spec's row in the `specs/README.md` index is in scope only when the spec's own §Documentation-updates section calls for it (e.g. a (d) retroactive Status flip). This index clause extends to the cross-repo own-repo index (per the cross-repo section's discover-it rule), not only `specs/README.md`; and a Step-6-Q-authorized cross-spec edit MAY additionally target that index's **content** (not just a §Documentation-updates status flip), recorded under the same "Cross-spec scope extension" line.

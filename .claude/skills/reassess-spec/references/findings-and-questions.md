...
- **Issues**: factually wrong, stale, violates a FOUNDATIONS §11 acceptance invariant, crosses a §12 stop condition, makes a §13 ADR-trigger decision without an ADR, or proposes redundant deliverables when existing infrastructure suffices. Blocks AGENT-TASK decomposition.
...
For each finding, record: what the spec says (or omits); what the codebase actually has (with file/crate paths and line references); the recommended change.

Tag severity: **CRITICAL** (blocks decomposition), **HIGH** (fix before decomposition), **MEDIUM** (improves quality), **LOW** (nice to fix).
...
**Pre-finalization site-counting**: for an Issue or Improvement that corrects a stale claim OR renames/eliminates a token (symbol-name, crate-name, tool-name, mechanism token) restated across multiple spec sections, grep the spec for ALL restatements of the claim/token BEFORE finalizing the finding's recommendation. List each site in the recommendation (e.g. `apply at §Scope + §Deliverables + §Exit-criteria row 3 + §Assumptions A-2`). Use a bare-token grep with `-i` (not the specific phrasing the finding expects to remove) to catch syntax variants. Audit-time enumeration is cheaper than the late-discovery recovery that the Step 7 pre-edit grep and Step 8 post-apply grep otherwise have to absorb.

## Step 6: Present Findings

**Redesign-count checkpoint** (before drafting): count deliverables (or Work-breakdown items) whose approach was materially changed — eliminated, replaced with a different mechanism, or restructured beyond a refinement. Express as `N / total`, where `total` is the **pre-reassessment** deliverable count (dropped deliverables stay in the denominator; added ones increase it; use the same denominator through every emission). **When a spec carries multiple parallel enumerations** (e.g. a §Design-decisions list, a §Deliverables list, AND a §Work-breakdown table), use the §Work-breakdown table as the canonical denominator — it is the decomposition unit — falling back to §Deliverables, then design-decisions; name which enumeration the ratio counts against. Emit the ratio as a one-line note in the `### Classification` block regardless of whether the flag fires. If `N/total > 50%`, the Substantial Redesign Flag section MUST appear immediately above Questions; otherwise omit it. **When the spec has no enumerated deliverable set** (e.g. a narrative spec with no Work-breakdown or Deliverables list), there is no denominator — state that explicitly (`Redesign count: denominator N/A — no enumerated deliverables; redesign assessed qualitatively as none | some | substantial`) rather than fabricating a ratio, and base the Substantial Redesign Flag decision on that qualitative judgment.

Material changes: elimination of a deliverable; replacement of its mechanism; restructuring that changes which crates/files it touches; changing the read/write direction of data flow; introducing a new authoritative state where the original was a derived view (or vice versa). NOT material: field renames, type-shape adjustments that preserve the deliverable's role, signature-preserving reorderings, prose rewording. A verification/evidence deliverable that re-anchors *only because* its drivers were redesigned counts once with its driver, not separately. For a multi-item inventory (a Work-breakdown table, an exit-criteria set), count each materially-changed sub-item toward the numerator but the container as one deliverable in the denominator.
...
[Only if a §12 stop condition is engaged. One line per engaged condition:]
- <condition>: clear | N/A | **flag** (reason)
...
- **Option fidelity**: each option that names a type/field/function/crate must cite its current definition (grepped at presentation time), not a summary. The user's approval binds to the option label, so an imprecise label produces an ambiguously approved fix.
...
- **Reform-depth question for non-canonical specs**: when Step 3.11 finds the spec missing most of the canonical section set (Header, Work breakdown, Exit criteria, Assumptions, alignment table all absent), OR finds the spec's content largely present but organized under a non-canonical structure/numbering (e.g. its own `§0`–`§N` scheme, or missing only a canonical Header), do not silently pick a restructuring depth. Present reform depth as an explicit discrete question — typically (a) full canonical rewrite; (b) graft the missing canonical sections — or, when the content is already present, a canonical Header plus a reader-orientation section map onto the existing body — while retaining the existing body as a detailed reference; (c) minimal (add only Work breakdown + Exit-criteria mapping, or — when those already exist — only the Header/status fix and any scaffolding strip). The output structure is a user decision with real effort tradeoffs; recommend a default (usually (b)).
...
- **Blanket approval of a recommendation-bearing Question**: when a Question carried an explicit recommendation and the user gives blanket approval ("approved", "go ahead", "lgtm", "ship it") without addressing the Question specifically, resolve it to the recommended option — the recommendation was the default and blanket approval accepts it. Record it in the Step 7 pre-apply table as `User answer Q<N> = recommendation accepted via blanket approval: <one-line paraphrase>`. An explicit objection or a named alternative still re-opens it. When the Question had no recommendation, blanket approval does not resolve it — treat it as still open and ask once more (one question at a time).
...
**Auto-mode interaction**: when auto mode (or an in-session no-stopping directive) is active AND the findings contain no Issues (CRITICAL/HIGH or FOUNDATIONS hard-fails) AND no open Questions, proceed directly to Step 7, reporting the auto-approval inline. If any Issue is present or any Question is open, the wait-for-user gate still applies; cite the directive inline when it holds.

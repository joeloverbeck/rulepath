...
For specs with >10 references, consider parallel Explore agents organized by theme (max 3). Spot-check agent claims with direct Grep/Read before including in findings — agent results are leads, not facts; trust a direct tool result over an agent claim. For **cross-repo / pinned-sibling** specs, prefer direct operator-controlled greps *split by repo* over Explore delegation regardless of count — repo-routing (spec's repo vs. pinned sibling) must be controlled, and a delegated agent may resolve paths against the wrong tree (SKILL.md §Worktree, Cross-Repo & Plan-Mode Awareness).
...
1. **Verify** each enumerated claim is adjudicated by the spec — **Accepted** in Scope/Deliverables/Work-breakdown with a per-claim mapping (this covers accept-with-divergence: the spec adopts the intent but deliberately diverges; record the divergence in the mapping), **Rejected** in Out-of-scope/Not-allowed with a rationale, or **Deferred** with a named follow-up gate. **Range/ellipsis bulk-adjudication**: a spec may adjudicate many homogeneous claims with one range token (e.g. `B-03…B-16`). Accept this when the spanned claims are homogeneous and route to a single deliverable — do **not** demand N separate adjudications — but verify the per-deliverable→claim mapping stays recoverable at decomposition, and when one-ticket-per-claim granularity will be needed (e.g. one ticket per template), raise a MEDIUM Improvement to add the explicit `claim-ID → target` mapping. Do not accept a range silently when the spanned claims are heterogeneous (they route to different deliverables or some are rejected/deferred) — those need individual adjudication.
2. **Surface unadjudicated claims** as MEDIUM Improvement findings — name the claim, cite the source line, recommend the spec add an adjudication. For ROADMAP gates specifically, every "Exit" and "Not allowed" line of the gate must be mapped: an unmapped ROADMAP exit criterion is a HIGH Issue (the spec's Exit-criteria section must mirror it row-for-row, per `specs/README.md`), and an unmapped "Not allowed" line is a HIGH Issue (it must carry into the spec's Not-allowed / Forbidden-changes sections). This row-for-row HIGH-Issue mapping applies **only when a ROADMAP gate actually backs the spec**; a post-gate correctness/contract follow-on with no ROADMAP gate (see the SKILL.md Pre-Process sub-type) has no gate exit-list to mirror — validate its Exit-criteria as self-originating from its own Reproduction / Objective instead, and do not fire the unmapped-ROADMAP-line Issue.
...
- **§Exit criteria + §Acceptance evidence** — exit criteria mapped row-for-row to the ROADMAP gate's exit list (or, for a post-gate correctness/contract follow-on with no ROADMAP gate — see the SKILL.md Pre-Process sub-type — self-originating exit criteria grounded in the spec's own Reproduction / Objective), and acceptance evidence naming re-runnable confirmation (tests, traces, replay/hash checks, benchmarks, simulations, boundary reviews) per `docs/TESTING-REPLAY-BENCHMARKING.md`. Use explicit `not applicable` rows over silent omissions. For a game gate, the evidence set must cover the `docs/OFFICIAL-GAME-CONTRACT.md` deliverables (rule coverage, golden traces, replay, visibility/no-leak, bot legality, benchmarks). A game spec missing these is a HIGH Issue.
- **§Forbidden changes + §Assumptions** — gate-specific prohibitions (carrying the ROADMAP "Not allowed" list, where a ROADMAP gate backs the spec; a no-ROADMAP follow-on instead carries the prohibitions implied by its own scope and the cited contracts) and one-line-correctable assumptions surfaced explicitly rather than discovered at task-time.
- **Internal cross-reference integrity** — grep the spec's own `§N` / `§N.M` cross-references against its actual `^#{2,3} N` headers. An internal ref that resolves to **no in-spec section** is a traceability finding — common in brief-derived specs that inherit the source document's numbering (e.g. a dangling `§3.6` handle for a decision the spec actually resolves in a differently-numbered section). Relabel it to the in-spec section, or add a one-line gloss tying the handle to its source document. Disambiguate from external-doc refs (FOUNDATIONS / ARCHITECTURE / ADR / `ref §NN`) before flagging — those legitimately point outside the spec.
- **Duplicate / colliding section numbers** — grep `^#{2,3} \d` headers for repeated numbers (e.g. two `§2.1` groups, two `§11.1` subsections, or unnumbered top-level sections interleaved among numbered ones). A collision is a structural finding that feeds the reform-depth question (`references/findings-and-questions.md`); for (a)/(b) specs it is at least a LOW Improvement, MEDIUM when it shadows the canonical decomposition sections (Work-breakdown / Exit-criteria).

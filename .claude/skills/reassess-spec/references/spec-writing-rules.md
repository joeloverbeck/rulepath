...
- **Grafting canonical sections onto a non-canonical body**: when adding missing canonical sections to a spec whose body already uses a conflicting numbering scheme (e.g. 22 narrative `## N.` sections), prefer a non-colliding scheme — lettered sections (`## A.`, `## B.`, …) plus a brief reader-orientation pointer in the Header — over renumbering the entire body. Renumber only when the body is already near-canonical. **Clean-append carve-out**: the lettered scheme exists to avoid a mid-body renumber; when a single canonical section is *appended after* a contiguous numeric body (no renumber needed), continuing the numeric scheme is fine (e.g. `## 21. Assumptions` after a contiguous `## 1.`–`## 20.` body) — reserve letters for mid-body insertions that would otherwise force a renumber.
- **Stripping a numbered scaffolding section under retained-numbering reform**: when the scaffolding strip (externally-generated specs; SKILL.md Pre-Process) removes a *section* that occupies a body section number — and reform option (b) retains the body's `§`-numbering — prefer repurposing that section number with the genuine content it wrapped (e.g. fold a baseline-findings subsection into it) over leaving a numbering gap (`## 1.` then `## 3.`). A true delete-with-gap is acceptable only when no genuine content survives the strip; otherwise the retained `§`-spine reads as if a section were lost.
- **Status value on a grafted Header**: when grafting a canonical Header onto an (a)/(b) spec that lacked one (or carried a non-standard status like `proposed maintenance spec`), set **Status** to the appropriate `specs/README.md` index value — `Not started` → `Planned` → `In progress` → `Done`. Default to `Planned` for a spec being prepared for decomposition. (The `Done` flip is the (d)-retroactive case only; see the Retroactive Branch below.)
...
- **Large contiguous deletion**: when removing a large contiguous block (e.g. a multi-screen evidence ledger) where reproducing it in an `Edit` `old_string` is impractical and a full `Write` of the whole spec is overkill, a scripted line-range deletion (e.g. `sed -i '<start>,<end>d'`) is acceptable after confirming the exact boundary lines. **After any out-of-tool (Bash) modification, Read the affected region again before the next `Edit`** — the Edit validator rejects a stale read and fails with "File has been modified since read." **When the deletion is at the top/early in the file (the common externally-generated ledger-strip) and other findings edit content below it, every downstream line number cited in the already-emitted pre-apply table goes stale by the deleted line count the moment the deletion lands.** Either sequence the large deletion last, or — preferred when document-order top→bottom is being followed — re-locate each downstream edit by symbol/string grep after the deletion rather than trusting the pre-deletion line numbers.
...
- **Site reconciliation before post-apply**: before drafting the §Post-Apply Confirmation, reconcile each pre-apply-table row's enumerated edit sites against the Edits actually issued — confirm every site the row named (e.g. `apply at §Scope + D10 + WB7`) received its edit, via one Edit per site or a count check. A site missed at apply-time otherwise surfaces only at the post-apply grep (e.g. an expected-3 corrected-reference count returning 2), forcing a recovery edit + re-grep round; the reconciliation catches it one step earlier.

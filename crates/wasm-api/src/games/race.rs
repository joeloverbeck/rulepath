...
use crate::seats::{
    canonical_seats_for_count, canonical_trace_seat_id, parse_replay_race_seat, seats,
};
...
pub(crate) fn race_replay_document_json(
    ...
    let commands_json = commands
        .iter()
        .enumerate()
        .map(|(index, command)| race_command_record_json(index, command))
...
fn race_command_record_json(index: usize, command: &AppliedCommand) -> Result<String, String> {
    let seat = parse_replay_race_seat(&command.actor_seat)?;

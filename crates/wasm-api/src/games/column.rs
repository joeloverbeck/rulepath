...
use crate::seats::{canonical_seats_for_count, canonical_trace_seat_id, parse_column_seat, seats};
...
pub(crate) fn column_replay_document_json(
    ...
    let commands_json = commands
        .iter()
        .enumerate()
        .map(|(index, command)| column_command_record_json(index, command))
...
fn column_command_record_json(index: usize, command: &AppliedCommand) -> Result<String, String> {
    let seat = parse_column_seat(&command.actor_seat)?;

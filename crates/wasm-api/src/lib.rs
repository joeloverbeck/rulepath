...
const GAME_EVENT_FRONTIER_DISPLAY_NAME: &str = "Event Frontier";
...
const EVENT_FRONTIER_TRACE_RULES_VERSION: &str = "event-frontier-rules-v1";
...
fn parse_event_frontier_seat(value: &str) -> Result<SeatId, String> {
    match value {
        "seat-0" | "seat_0" => Ok(SeatId("seat_0".to_owned())),
        "seat-1" | "seat_1" => Ok(SeatId("seat_1".to_owned())),
        _ => Err(format!(
...
fn event_frontier_actor_for_seat(
    state: &EventFrontierState,
    seat: &SeatId,
) -> Result<Actor, String> {
    if state.seats.iter().any(|candidate| candidate == seat) {
...
fn event_frontier_viewer_for_seat(
    state: &EventFrontierState,
    seat: Option<&str>,
) -> Result<Viewer, String> {
...
fn event_frontier_viewer_authorizes_actor(
    viewer_seat: Option<&str>,
    actor: &SeatId,
) -> Result<bool, String> {
    viewer_seat

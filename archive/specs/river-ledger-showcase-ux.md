...
Completed tickets: 17 archived tickets, `archive/tickets/RIVLEDSHOWUX-001.md` through `archive/tickets/RIVLEDSHOWUX-017.md`.

Implementation summary: River Ledger now ships Rust-authored clean seat/action/board/ledger copy, strengthened runtime no-raw-seat guards, V2 showdown presentation with ranked standings, decisive contrast, folded rows, board-once usage marks and live-region status, central board-well table recomposition for 3-6 seats, River-scoped `--rl-*` tokens, scheduler-routed contribution/street/showdown feedback, viewer-safe bot `Why?`, and an original River Ledger catalog icon. TypeScript renders Rust/WASM payloads and layout only; legality, evaluation, standings, card usage, bot reasons, reveal scope, and action-row relevance remain Rust-owned.

Docs/status summary: `games/river_ledger/docs/UI.md`, `games/river_ledger/docs/RULE-COVERAGE.md`, `apps/web/README.md`, and `specs/README.md` were reconciled in RIVLEDSHOWUX-017. The active spec index row was flipped to `Done` with archived-ticket evidence.

Verification evidence:

- Rust and per-game: `cargo test --workspace`; `cargo run -p fixture-check -- --game river_ledger`; `cargo run -p replay-check -- --game river_ledger --all`; `cargo run -p rule-coverage -- --game river_ledger`; `bash scripts/boundary-check.sh`.
- Docs/catalog: `node scripts/check-doc-links.mjs`; `node scripts/check-catalog-docs.mjs`.
- Web: `npm --prefix apps/web run build`; `npm --prefix apps/web run smoke:wasm`; `npm --prefix apps/web run smoke:ui`; `npm --prefix apps/web run smoke:effects`; `npm --prefix apps/web run smoke:e2e`.
- `npm --prefix apps/web run smoke:effects` first passed the Node effect-feedback portion, then its browser animation segment failed to bind `127.0.0.1` with `listen EPERM` under the sandbox. The exact command was rerun with localhost/browser escalation and passed.

Manual/non-command evidence: no persistent screenshot artifacts were added. The visual/display-size checks requested by the leaf tickets were covered by browser assertions in the e2e smokes, including River Ledger responsive table checks, V2 showdown/card-usage rendering, bot `Why?` DOM checks, and shell verification that the dedicated catalog icon renders with a catalog-derived accessible title rather than the generic fallback.

Archive truthing: before reference archival, the active `tickets/RIVLEDSHOWUX*.md` glob was empty after RIVLEDSHOWUX-017 was archived; archived ticket status/outcome guards passed per ticket. The only unrelated working-tree changes left untouched were `.claude/skills/spec-to-tickets/SKILL.md` and `.claude/skills/spec-to-tickets/references/decomposition-patterns.md`.

...
**Priority**: MEDIUM
**Effort**: Small
**Engine Changes**: Yes (dev-only profile adapter) — `games/secret_draft/Cargo.toml` `[dev-dependencies]`
**Deps**: 014

## Problem

Spec §3.10 / task `8C-R2-501`: Secret Draft must list `game-test-support` as a **dev-only** dependency to enable the C-07 no-leak harness and C-08 profile drivers. R2 adds it under `[dev-dependencies]` only; no normal/build/WASM/tool edge may be created. Shares `Cargo.toml` with `-014` (hence `Deps: 014`).

## Assumption Reassessment (2026-06-23)

1. `games/secret_draft/Cargo.toml` does not currently depend on `game-test-support` (confirmed in the reassess pass); HCD already has the dev-dependency, so only Secret/Poker/Masked need it added.
2. Spec §3.10/§3.12/§9: dev-dependency only; a normal/build/WASM/browser/tool edge on `game-test-support` is forbidden.
3. Cross-crate boundary under audit: the `game-test-support` dev-only edge — it supplies `no_leak`/`profiles` harnesses to tests only (`crates/game-test-support/src/{no_leak,profiles}.rs`), never production code.
...
1. Dev-only edge, no normal edge -> dependency check (`cargo tree --workspace -e normal --invert game-test-support` shows no `secret_draft` edge) + `bash scripts/boundary-check.sh`.
2. Tests can resolve the harness -> `cargo test -p secret_draft`.
...
Add `game-test-support` under `[dev-dependencies]` in `games/secret_draft/Cargo.toml`.

## Files to Touch

- `games/secret_draft/Cargo.toml` (modify; serialized after `-014`)
- `Cargo.lock` (modify; records the dev-dependency edge)

## Out of Scope

- Any test that *uses* the harness (C-07 `-025`, C-08 profile drivers `-029`/`-033`/`-037`/`-040`).
...
1. `cargo test -p secret_draft` green (harness resolves at dev time).
2. `cargo tree --workspace -e normal --invert game-test-support` shows no normal `secret_draft` edge; `bash scripts/boundary-check.sh` passes.
...
Added `game-test-support` under `games/secret_draft/Cargo.toml`
...
1. `cargo tree --workspace -e normal --invert game-test-support` showed only
   the `game-test-support` package itself, with no `secret_draft` normal edge.

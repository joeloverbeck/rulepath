# 8CR1PUBFIXSEA-009: Column Four C-02 strict canonical seat parser

**Status**: COMPLETED
**Priority**: MEDIUM
**Effort**: Small
**Engine Changes**: Yes — `games/column_four` (`src/ids.rs`); accepted canonical input unchanged, no trace/golden change
**Deps**: 8CR1PUBFIXSEA-001

## Problem

`games/column_four/src/ids.rs::ColumnFourSeat::parse` hand-rolls canonical seat parsing (`match value { "seat_0" => …, "seat_1" => …, _ => None }`) instead of delegating to `SeatId::parse_canonical` (MSC-8C-002). Adopt strict canonical delegation plus a bounded enum mapping, preserving accepted values and `as_str()`, and add direct rejection tests. The WASM import adapter (`parse_seat_import`) remains the only place bounded legacy aliases are accepted.

## Assumption Reassessment (2026-06-23)

1. `ColumnFourSeat::parse` currently matches the canonical underscore strings directly and `ColumnFourSeat::as_str()` returns them; `SeatId::parse_canonical` exists in `crates/engine-core/src/lib.rs`. Confirmed during reassessment.
...
3. Cross-artifact: canonical seat identity is an `engine-core` contract (`SeatId`); the game maps it to typed `ColumnFourSeat`. No TypeScript normalization (§2).
...
Rewrite `ColumnFourSeat::parse` to call `SeatId::parse_canonical` and map the bounded result to the typed enum; preserve `as_str()` exactly.
...
1. `cargo test -p column_four` is green, including new malformed/out-of-range rejection tests.
2. `cargo test -p wasm-api` is green — `parse_seat_import` still accepts bounded aliases.
3. `cargo run -p replay-check -- --game column_four --all` passes with all downstream hashes/visibility unchanged.
...
1. `games/column_four/src/ids.rs` parser unit tests — canonical round-trip plus leading-zero/Unicode-digit/hyphen/symbolic/out-of-range rejection.
...
- Rewrote `ColumnFourSeat::parse` to delegate to `SeatId::parse_canonical`, then map the bounded canonical index through `ColumnFourSeat::from_index`.
- Preserved `ColumnFourSeat::as_str()` output exactly (`seat_0`, `seat_1`).
...
- `cargo test -p column_four` passed, including the new parser acceptance/rejection tests.
- `cargo test -p wasm-api` passed, including `seats::tests::import_adapter_accepts_canonical_hyphen_and_symbolic_aliases`.
- `cargo run -p replay-check -- --game column_four --all` passed; all Column Four traces reported `ok`.
- Grep proof confirms `ColumnFourSeat::parse` calls `SeatId::parse_canonical`, and non-canonical examples remain rejection-only in the game parser tests.

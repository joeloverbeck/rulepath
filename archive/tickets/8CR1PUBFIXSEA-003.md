# 8CR1PUBFIXSEA-003: Three Marks C-01 public-envelope constructor adoption

**Status**: COMPLETED
**Priority**: MEDIUM
**Effort**: Small
**Engine Changes**: Yes — `games/three_marks` (`src/effects.rs`); effect bytes and visibility byte-identical
**Deps**: 8CR1PUBFIXSEA-001

## Problem

`games/three_marks/src/effects.rs::public_effect` constructs the public effect envelope with a `VisibilityScope::Public` struct literal instead of the shipped kernel constructor `EffectEnvelope::public` (MSC-8C-001). Adopt the constructor with zero change to payload, effect order, or visibility — an ADR-0009 `unchanged` adoption.

## Assumption Reassessment (2026-06-23)

1. `games/three_marks/src/effects.rs::public_effect` currently builds `EffectEnvelope { visibility: VisibilityScope::Public, payload }` as a literal; `EffectEnvelope::public(payload)` exists as an inherent constructor in `crates/engine-core/src/lib.rs`. Confirmed during reassessment.
...
5. Enforcement surface = effect serialization/hash and the Three Marks replay traces; the constructor swap produces identical bytes (ADR-0009 `unchanged`), leaking no hidden information and breaking no deterministic replay/hash.
...
1. Effect bytes/hash unchanged -> deterministic replay-hash check (`replay-check --game three_marks --all`).
2. Public visibility preserved -> focused public-visibility assertion in the Three Marks effect/replay tests.
...
- `games/three_marks/src/effects.rs` (modify)
...
1. `cargo test -p three_marks` is green, including a focused before/after public-effect equality assertion.
2. `cargo run -p replay-check -- --game three_marks --all` passes with effect/public-view hashes byte-identical to the `-001` baseline.
...
1. `games/three_marks/` effect/replay test module — add (or strengthen) a focused assertion that `public_effect` yields a `Public`-visibility envelope with the baseline payload bytes.
...
Implemented the C-01 Three Marks public-envelope constructor adoption by
replacing the local `EffectEnvelope { visibility: VisibilityScope::Public,
payload }` literal in `games/three_marks/src/effects.rs::public_effect` with
`EffectEnvelope::public(payload)`. Added a focused unit test proving public
visibility and payload preservation.

Deviations from the original plan:

- Added a new `effects.rs` test module because Three Marks had no existing
  effects-local test block for this exact helper.

Verification:

- `cargo fmt --all -- --check` passed.
- `cargo test -p three_marks` passed.
- `cargo run -p replay-check -- --game three_marks --all` passed.
- `bash scripts/boundary-check.sh` passed.
- `rg -n "EffectEnvelope::public|EffectEnvelope \\{|VisibilityScope::Public" games/three_marks/src/effects.rs` confirmed the public helper now calls `EffectEnvelope::public`, with `VisibilityScope::Public` only in the focused test.

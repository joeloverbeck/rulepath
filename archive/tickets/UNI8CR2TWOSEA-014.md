...
Spec §3.6 / task `8C-R2-302`: `setup_match` validates seat structure with a hand-written length predicate against `STANDARD_SEAT_COUNT`. R2 replaces only that predicate with structural `SeatCount` use, adding a normal `game-stdlib` dependency, and preserves diagnostics and state bytes.

## Assumption Reassessment (2026-06-23)

1. `games/secret_draft/src/setup.rs::setup_match` exists (confirmed ~line 22) and compares `seats.len()` against the game-local `STANDARD_SEAT_COUNT`; `secret_draft` does **not** currently depend on `game-stdlib` (confirmed — this ticket adds it).
...
3. `engine-core` stays noun-free; `game-stdlib` use is the already-earned `SeatCount` (no ring policy). The new dependency is a normal edge (not dev-only), distinct from the C-06 dev-dependency in `-021`.

## Verification Layers

1. Accepted count 2, rejected counts with the exact diagnostic -> `cargo test -p secret_draft` (setup tests).
2. State hashes unchanged -> deterministic replay-hash check (`replay-check --game secret_draft --all`).
...
Add `game-stdlib` under `[dependencies]` in `games/secret_draft/Cargo.toml`.
...
- `games/secret_draft/Cargo.toml` (modify)
- `games/secret_draft/src/setup.rs` (modify)

## Out of Scope

- The C-06 `game-test-support` dev-dependency (`-021`).
...
1. `cargo test -p secret_draft` green, with accepted/rejected seat-count cases.
2. `cargo run -p replay-check -- --game secret_draft --all` — state hashes byte-identical to baseline.
...
1. `games/secret_draft/src/setup.rs` (or `tests/`) — accepted/rejected structural assertions with the exact diagnostic.

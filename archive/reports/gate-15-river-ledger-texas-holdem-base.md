...
| Stage | Stage 15 / Public scaling phase |
| Gate | Gate 15 |
| Status | `Planned` |
| Date | 2026-06-14 |
| Owner | Rulepath maintainers |
| Target repository | `joeloverbeck/rulepath` |
| Target commit used for repository evidence | `c378be25066c01423dbbc995b21bd8c071b7e2fe` |
| Freshness claim | User-supplied target commit only; this spec does **not** verify that the commit is current `main`. |
...
| New artifact | Yes. The uploaded manifest has no existing `games/river_ledger/` crate and no existing Gate 15 spec path. |
| Commit mismatch note | The research brief names commit `47e202a`; the exact-commit discipline guardrail supplied with this task names `c378be25066c01423dbbc995b21bd8c071b7e2fe`. This spec uses the guardrail commit as the target of record and treats the manifest as path inventory only. |
...
| WASM crate | Add dependency in `crates/wasm-api/Cargo.toml`; import the crate in `crates/wasm-api/src/lib.rs`; add `RegisteredGame::RiverLedger`; update `list_games`; update catalog metadata/seat labels/viewer modes; update `MatchRecord`; update setup/action/view/effects/replay/bot dispatch; update `pairwise_no_leak_result` and `assert_pairwise_no_leak` dispatch. |
| Simulator | Add `river_ledger` to `tools/simulate` dependency list and game dispatch. Ensure summaries use the existing `wins_by_seat: BTreeMap` shape for 3–6 seats. |
...
| Deliverables | `benches/river_ledger.rs`; `benches/thresholds.json`; simulator fixtures; `docs/BENCHMARKS.md`. |
...
| Deliverables | Root workspace member; `ci/games.json`; `crates/wasm-api` dependency/import/enum/list/catalog/match dispatch; `tools/simulate`; `tools/replay-check`; `tools/fixture-check`; `tools/rule-coverage` with `RL-*` prefix support. |
...
cargo run -p simulate -- --game river_ledger --seat-count 3 --games 1000 --seed 1503
cargo run -p simulate -- --game river_ledger --seat-count 4 --games 1000 --seed 1504
cargo run -p simulate -- --game river_ledger --seat-count 5 --games 1000 --seed 1505
cargo run -p simulate -- --game river_ledger --seat-count 6 --games 1000 --seed 1506
...
Add the new spec path to the living index at Order 5 / Gate 15 with status `Planned` when this spec is admitted. Flip to `Done` only after the Gate 15 exit criteria and release checklist are satisfied.

Paste-ready row shape:
...
2. The target commit of record for this spec is the exact-commit guardrail value `c378be25066c01423dbbc995b21bd8c071b7e2fe`; no latest-main claim is made.
3. The uploaded manifest is treated as path inventory only and shows River Ledger as a greenfield crate/spec at the target evidence set.
4. The Rulepath v1 fixed-limit cap is one opening bet plus three raises per street. If maintainers prefer four raises after the opener, they must update `RL-BET-*` rules, traces, bots, and benchmarks before ticket decomposition.
5. Burn cards are optional internal deterministic deck advancement. If implemented, they are never visible; if omitted, source notes must say River Ledger abstracts them away without changing public rules outcomes.
6. A single-hand match is sufficient for Gate 15 base. Button rotation must still be deterministic and documented so later multi-hand or tournament-like variants do not invent incompatible semantics.
7. Contribution units are abstract counters with high capacity. Any later capacity/all-in behavior belongs to Gate 15.1.
8. Royal flush is treated as the highest straight flush unless maintainers choose to expose it as a presentation alias in `RULES.md`; either way the evaluator comparison is identical.
9. External source references verify public rules facts only. They do not license copied prose, copied assets, casino presentation, or product mimicry.

...
use directional_flip::{
    apply_action, legal_action_tree, legal_placements, placement_flips, project_view,
    replay_support::{project_step, replay_commands, DirectionalFlipReplayJson},
    setup_match, validate_command, DirectionalFlipLevel2Bot, DirectionalFlipRandomBot,
    DirectionalFlipState, SetupOptions,
};
use engine_core::{
...
const GAME_ID: &str = "directional_flip";
...
const THRESHOLDS: &[Threshold] = &[
    threshold("legal_actions", "trees_per_second"),
    threshold("flip_scanning", "scans_per_second"),
    threshold("apply_action", "actions_per_second"),
    threshold("public_view_generation", "views_per_second"),
    threshold("replay_step_projection", "projections_per_second"),
    threshold("serialization_roundtrip", "roundtrips_per_second"),
...
const fn threshold(operation: &'static str, unit: &'static str) -> Threshold {
    Threshold {
        ...
        caveat: "Smoke floor only until directional_flip has stable CI measurements.",
    }
}
...
fn main() {
    ...
    println!("BEGIN_DIRECTIONAL_FLIP_BENCHMARK_JSON");
    println!("{}", benchmark_report_json(&metadata, &results));
    println!("END_DIRECTIONAL_FLIP_BENCHMARK_JSON");
}
...
fn run_benchmarks(filter: Option<&str>) -> Vec<BenchResult> {
    let benches: Vec<BenchSpec> = vec![
        ("legal_actions", "trees", 50_000, bench_legal_actions),
        ("flip_scanning", "scans", 50_000, bench_flip_scanning),
        ("apply_action", "actions", 50_000, bench_apply_action),
        ...
        ("random_playout", "games", 1_000, bench_random_playout),
        (
            "level0_bot_decision",
            "decisions",
            25_000,
...
fn print_human_summary(results: &[BenchResult]) {
    println!("directional_flip native benchmarks");
...
fn benchmark_report_json(metadata: &ReportMetadata, results: &[BenchResult]) -> String {
    ...
    format!(
        concat!(
            ...
            "  \"schema_version\": {},\n",
            "  \"game_id\": \"{}\",\n",
            "  \"rules_version\": \"directional_flip-rules-v{}\",\n",
            "  \"data_version\": \"{}\",\n",
            "  \"engine_version\": \"{}\",\n",
            "  \"build_profile\": \"{}\",\n",
            "  \"command\": \"{}\",\n",
...
fn bench_flip_scanning(iterations: u64) {
    let state = initial_state(2);
    for _ in 0..iterations {
        black_box(legal_placements(
            black_box(&state),
            black_box(state.active_seat),
        ));
        black_box(placement_flips(
            black_box(&state),
            black_box(state.active_seat),
            black_box(directional_flip::CellId::new(
                directional_flip::RowId::R3,
                directional_flip::ColumnId::C4,
            )),
        ));
    }
}
...
fn bench_replay_step_projection(iterations: u64) {
    ...
    let command = command_for_state(&state, "place/r3c4");
    let action = validate_command(&state, &command).expect("benchmark command validates");
...
fn bench_serialization_roundtrip(iterations: u64) {
    let replay = DirectionalFlipReplayJson {
        ...
        command_segments: vec!["place/r3c4".to_owned(), "place/r3c5".to_owned()],
    };
    let json = replay.to_json();
    for _ in 0..iterations {
        let parsed = DirectionalFlipReplayJson::from_json(black_box(&json)).expect("replay parses");
...
fn bench_level2_bot_decision(iterations: u64) {
    let state = initial_state(8);
    let bot = DirectionalFlipLevel2Bot::new(Seed(8));
    for _ in 0..iterations {
        black_box(
...
fn run_random_playout(seed: u64) -> u64 {
    ...
    while state.terminal_outcome.is_none() {
        let bot = DirectionalFlipRandomBot::new(Seed(seed.wrapping_add(actions)));
        let action_path = bot
...
fn initial_state(seed: u64) -> DirectionalFlipState {
...
fn actor_for_state(state: &DirectionalFlipState) -> Actor {
    Actor {
        seat_id: state.seats[state.active_seat.index()].clone(),
    }
}

fn command_for_state(state: &DirectionalFlipState, segment: &str) -> CommandEnvelope {
    CommandEnvelope {
        actor: actor_for_state(state),
...
fn first_legal_segment(state: &DirectionalFlipState) -> String {

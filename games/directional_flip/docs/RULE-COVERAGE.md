...
| `DF-END-001` | A placement fills the board (`board_full` terminal trigger). | terminal checks in `rules.rs`/`actions.rs`. | `full-board-terminal.trace.json`; terminal rule tests. | covered-by-trace | Final counts decide win or draw. |
| `DF-END-002` | A placement leaves no possible continuation before the board is full (`no_continuation` trigger). | terminal checks in `rules.rs`. | terminal rule tests; `double-pass-terminal.trace.json`. | covered-by-trace | Final counts decide win or draw. |
| `DF-END-003` | Two consecutive forced passes end the match (`double_forced_pass` trigger). | pass terminal branch in `rules.rs`. | `double-pass-terminal.trace.json`; rule tests. | covered-by-trace | Double-pass trace is required evidence. |

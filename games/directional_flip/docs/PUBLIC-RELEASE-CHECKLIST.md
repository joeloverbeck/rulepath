# Directional Flip Public Release Checklist
...
If content ships to an unauthorized browser, it has shipped. Directional Flip's release review covers source files, browser payloads, replay exports, DOM attributes, storage, console output, UI copy, and screenshots/demo recordings made from the app.
...
| variant and deviations are clear | pass | standard 8 by 8, two seats, four center discs, explicit forced pass |
| no copied rulebook prose | pass | original docs |
| no copied component text/assets/screenshots/scans/trade dress | pass | CSS/SVG board is project-authored and neutral |
| public name/trademark/trade-dress risk reviewed | pass | neutral `Directional Flip`; no proprietary name or palette |
...
| source maps if shipped | not shipped | no | default production output checked by smoke |
...
| action tree | pass | Rust action tree tests; UI smoke | legal placements or forced pass only |
| previews | pass | preview/apply tests; browser smoke | Rust target and ordered flips only |
...
| DOM attributes/test IDs | pass | `directional-flip.smoke.mjs` | no forbidden leak vocabulary |
| browser console/logs | pass | `directional-flip.smoke.mjs` | console/page errors scanned |
...
| bot explanations | pass | bot tests; Directional Flip smoke | public rationale only |
...
| Check | Status | Evidence/notes |
|---|---|---|
| replay reproduces deterministic hashes | pass | `cargo run -p replay-check -- --game directional_flip --all` |
...
| Check | Status | Evidence/notes |
|---|---|---|
| default public screen is play-first | pass | shell smoke and Directional Flip smoke |
| visual target is polished and neutral | pass | [UI.md](UI.md), `DirectionalFlipBoard.tsx` |
| no proprietary mimicry/trade dress | pass | neutral name/colors/shapes |
| legal moves are obvious | pass | legal cell highlights from Rust legal targets |
| player feedback is clear after every action | pass | semantic effects/status/score/bot rationale |
| semantic effects drive animations | pass | placed/flipped classes from Rust effect log |
...
| Check | Status | Evidence/notes |
|---|---|---|
| keyboard path for core actions | pass | `directional-flip.smoke.mjs` moves grid focus and presses Enter |
| forced-pass path | pass | bot-vs-bot smoke reaches Rust forced pass |
...
| color is not sole information channel | pass | SVG marks/patterns, labels, score/status text |
| reduced-motion behavior implemented | pass | Directional Flip smoke checks flip animation disabled |
...
|---|---|---|
| unit/named rule tests | pass | `cargo test -p directional_flip` |
| golden traces | pass | 14 trace files; replay-check |
| invalid/stale diagnostic traces | pass | occupied, non-flipping, non-active, stale diagnostics |
| forced-pass traces | pass | forced pass and double-pass terminal traces |
| property/invariant tests | pass | `games/directional_flip/tests/property.rs` |
| simulation/fuzz runs | pass | `cargo run -p simulate -- --game directional_flip --games 1000` |
| replay/hash tests | pass | `cargo run -p replay-check -- --game directional_flip --all` |
...
| bot legality/determinism/explanation tests | pass | `cargo test -p directional_flip` |
| UI smoke tests | pass | Directional Flip and shared E2E smokes |
| native benchmarks | pass with caveat | `cargo bench -p directional_flip`; thresholds are smoke floors |
...
- Gate 6 implementation and verification evidence is complete.
...
- Primitive-pressure third-use review is recorded without promoting speculative shared helpers.

Release constraints:

- Complete the final status/picker exposure ticket before declaring the gate closed.
...
|---|---|---|---:|
| final status/picker exposure pending | GAT6DIRFLI-021 | project | yes for public catalog exposure |

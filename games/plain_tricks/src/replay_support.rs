...
use crate::{
    ...
    setup_match, validate_command, Phase, PlainTricksEffect, PlainTricksSeat, PlainTricksState,
    SetupOptions, TerminalOutcome, TrickCardId, ACTION_PLAY, GAME_ID, RULES_VERSION_LABEL,
    VARIANT_ID,
};
...
pub struct PlainTricksInternalTrace {
...
pub struct ReplayResult {
    ...
    pub final_state: PlainTricksState,
}
...
impl StableSerialize for PlainTricksInternalTrace {
...
pub fn generate_internal_full_trace() -> PlainTricksInternalTrace {
    trace_from_seeded_first_legal_playout(0)
}
...
pub fn trace_from_commands(
    ...
    PlainTricksInternalTrace {
        ...
        commands: commands
            .iter()
            .map(|(seat, card)| ReplayCommand {
                actor: seat.as_str().to_owned(),
                path: vec![ACTION_PLAY.to_owned(), card.as_str().to_owned()],
            })
            .collect(),
    }
}

pub fn trace_from_seeded_first_legal_playout(seed: u64) -> PlainTricksInternalTrace {
    ...
    while state.phase != Phase::Terminal {
        ...
        let card = first_legal_card(&state, actor);
        let command = command_for_state(
            &state,
            actor,
            vec![ACTION_PLAY.to_owned(), card.as_str().to_owned()],
        );
        let action = validate_command(&state, &command).expect("generated command validates");
        apply_action(&mut state, action).expect("generated command applies");
...
pub fn replay_internal_full_trace(trace: &PlainTricksInternalTrace) -> ReplayResult {
    let mut state = setup_match(
    ...
    for replay_command in &trace.commands {
        let actor = PlainTricksSeat::parse(&replay_command.actor).expect("trace actor is valid");
...
pub fn export_public_replay(
    trace: &PlainTricksInternalTrace,
    viewer: &Viewer,
) -> PublicReplayExport {
    let mut state = setup_match(
        engine_core::Seed(trace.seed_evidence),
...
pub fn state_hash(state: &PlainTricksState) -> HashValue {
    HashValue::from_stable_bytes(state.stable_internal_summary().as_bytes())
}

pub fn view_hash(state: &PlainTricksState, viewer: &Viewer) -> HashValue {
...
pub fn effect_hash(effects: &[EffectEnvelope<PlainTricksEffect>]) -> HashValue {
    let bytes = effects
        .iter()
...
pub fn effect_stable_string(effect: &EffectEnvelope<PlainTricksEffect>) -> String {
...
fn redacted_command_summary(actor: PlainTricksSeat) -> String {
    format!("{}:play_action_redacted", actor.as_str())
}

fn first_legal_card(state: &PlainTricksState, actor: PlainTricksSeat) -> TrickCardId {
    legal_action_tree(
        state,
        &Actor {
            seat_id: state.seats[actor.index()].clone(),
    ...
    .and_then(|choice| TrickCardId::parse(&choice.segment))
    .expect("active actor has legal card")
}

fn choice_summary(choice: &engine_core::ActionChoice) -> String {
    let next = choice
        .next
        .as_ref()
        .map(|node| {

//! Replay/export support for Event Frontier.
//!
//! Internal traces may carry full deck order under native-test authority.
//! Public exports are viewer-scoped observations and never carry undrawn order.
...
pub const TRACE_HIDDEN_SURFACE: &str = "undrawn_deck_order";
pub const TRACE_STOCHASTIC_SURFACE: &str = "setup_shuffle";

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct EventFrontierInternalTrace {
    ...
    pub hidden_surface: String,
    pub stochastic_surface: String,
    pub per_seat_hidden_surface: String,
...
pub fn generate_internal_full_trace(
    seed: u64,
    state: &EventFrontierState,
) -> EventFrontierInternalTrace {
    let mut full_deck_order = Vec::new();
    full_deck_order.extend(state.deck.current.map(|card| card.as_str().to_owned()));
    full_deck_order.extend(state.deck.next_public.map(|card| card.as_str().to_owned()));
    full_deck_order.extend(
        state
            .deck
            .undrawn
            .iter()
    ...
    EventFrontierInternalTrace {
        ...
        full_deck_order,
        hidden_surface: TRACE_HIDDEN_SURFACE.to_owned(),
        stochastic_surface: TRACE_STOCHASTIC_SURFACE.to_owned(),
        per_seat_hidden_surface: "not_applicable".to_owned(),
...
pub fn public_replay_step(
    step_index: usize,
    state_after_step: &EventFrontierState,
...
pub fn export_public_replay(
    ...
    PublicReplayExport {
        ...
        hidden_information: TRACE_HIDDEN_SURFACE.to_owned(),
        hidden_information_redaction: "undrawn_order_redacted".to_owned(),
        stochastic_game_rule_events: TRACE_STOCHASTIC_SURFACE.to_owned(),
        steps,
    }
}
...
impl EventFrontierInternalTrace {
    pub fn stable_summary(&self) -> String {
        format!(
            "schema={};game={};rules={};seed={};variant={};deck={};hidden={};stochastic={};per_seat={};initial={}",
            self.schema_version,
            self.game_id,
            ...
            self.hidden_surface,
            self.stochastic_surface,
            self.per_seat_hidden_surface,
            self.initial_state_summary
        )
    }
}

impl StableSerialize for EventFrontierInternalTrace {
...
impl PublicReplayExport {
    pub fn stable_summary(&self) -> String {
        format!(
            "schema={};game={};rules={};variant={};viewer={};hidden={};redaction={};stochastic={};steps={}",
            self.schema_version,
            self.game_id,
            self.rules_version_label,
            self.variant,
    ...
    pub fn to_json(&self) -> String {
        format!(
            "{{\"schema_version\":{},\"game_id\":\"{}\",\"rules_version_label\":\"{}\",\"variant\":\"{}\",\"viewer\":\"{}\",\"hidden_information\":\"{}\",\"hidden_information_redaction\":\"{}\",\"stochastic_game_rule_events\":\"{}\",\"steps\":[{}]}}",
            self.schema_version,
            escape_json(&self.game_id),
            escape_json(&self.rules_version_label),
            escape_json(&self.variant),
            escape_json(&self.viewer),
            escape_json(&self.hidden_information),
...
fn reject_unknown_json_keys(input: &str, allowed: &[&str]) -> Result<(), String> {
    ...
    while index < bytes.len() {
        ...
        if cursor < bytes.len() && bytes[cursor] == b':' && !allowed.contains(&key) {
            return Err(format!("unknown JSON key `{key}`"));
        }
        index = key_end + 1;
    }
    Ok(())

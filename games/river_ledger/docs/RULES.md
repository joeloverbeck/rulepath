...
implement real-money features, tournament structure, rake, payouts, no-limit
play, pot-limit play, hosted multiplayer, or casino presentation. Gate 15.1 v2
adds finite abstract stacks, all-in states, side pots, uncalled returns, and
per-pot allocation inside the same fixed-limit River Ledger variant.
...
| terminal outcomes | last-live-hand foldout, showdown winner, split showdown, per-pot side-pot allocation |
| randomness | deterministic seeded shuffle only |
| excluded scope | no-limit, pot-limit, tournaments, real-money features |
...
| `RL-POT-SINGLE-001` | base pot | The base public contribution total. | public | Superseded for v2 terminal accounting by ordered pot tiers and `RL-POT-LAYER-*`. |
...
| `RL-STREET-SHOWDOWN-001` | Showdown | Evaluate all showdown-eligible live seats and allocate the eligible pot or pots. | Terminal outcome is recorded. |
...
## Gate 15.1 finite-stack and all-in rules

| Rule ID | Stack/all-in rule | Timing | Notes |
|---|---|---|---|
| `RL-STACK-SETUP-001` | Setup accepts an ordered per-seat starting-stack vector or uses the equal default stack. | setup | The vector length must equal the seat count and each value must be bounded. |
| `RL-STACK-SETUP-002` | Small and big blinds are capped by the posting seat's remaining stack. | setup | A blind can put a short seat immediately all-in without underflow. |
| `RL-ALLIN-CAP-001` | A legal action never contributes more than the actor's remaining stack. | action validation/application | Stack before, stack after, added amount, and all-in status are Rust-authored metadata. |
| `RL-ALLIN-CALL-001` | A seat unable to cover the full call price may fold or call all-in for its entire remaining stack. | facing a contribution | The action family remains `Call`; all-in is action metadata, not a separate arbitrary amount action. |
| `RL-ALLIN-BET-001` | A short opening bet, short raise, exact-stack full bet, or exact-stack full raise may consume the actor's stack. | legal bet/raise spots | Rust distinguishes short all-in pressure from full fixed-unit pressure. |
| `RL-ALLIN-REOPEN-001` | An incomplete all-in increase smaller than the current street unit does not by itself reopen raising; cumulative incomplete pressure reopens only after reaching one full unit. | per street/per previously acted seat | River Ledger deliberately uses a full-unit reopening rule for this fixed-limit variant. |
| `RL-ALLIN-ACT-001` | Folded and all-in seats are skipped by actor rotation and have no normal legal actions. | actor selection/action generation | If no decisions remain and at least two eligible seats remain, Rust runs out the board deterministically to showdown. |

## Side-pot and allocation rules

| Rule ID | Pot/accounting rule | Timing | Notes |
|---|---|---|---|
| `RL-POT-SINGLE-002` | The Gate 15 single-pot rule is superseded by ordered contribution layers when stacks differ. | whole hand | Existing equal-stack single-pot behavior remains a degenerate one-layer case. |
| `RL-POT-ALLIN-001` | The Gate 15 all-in-absent rule is superseded by stack-capped all-in action metadata. | whole hand | The legacy ID remains documented for migration traceability. |
| `RL-POT-LAYER-001` | Final pots are built from ascending contribution caps. | terminal allocation and live projection | Each layer has deterministic `main_pot`, `side_pot_1`, `side_pot_2`, ... identifiers. |
| `RL-POT-LAYER-002` | Folded seats' committed contributions remain in pot amounts but folded seats are excluded from eligibility. | layer construction | Folded money stays public accounting; folded private cards remain hidden. |
| `RL-POT-LAYER-003` | Adjacent contribution layers with identical eligibility coalesce deterministically. | layer construction | Coalescing preserves conservation and stable pot ordering. |
| `RL-POT-RETURN-001` | A singleton top contribution layer is returned to the only contributor before pot allocation. | terminal allocation/projection | Returned excess is public accounting and is not awarded as a pot share. |
| `RL-POT-ALLOC-001` | Each pot is evaluated independently against only its eligible seats. | showdown terminal | Different pots may have different winners. |
| `RL-POT-ALLOC-002` | Ties and odd units are resolved independently per pot by stable button order among that pot's tied winners. | showdown terminal | Button order assigns remainders only; it does not alter winner identity. |
| `RL-OUTCOME-POTPRESENT-001` | Public terminal outcome explains pot tiers, eligibility, returns, winners, shares, and final allocations without TypeScript recomputation. | terminal projection/UI | Rust is the only authority for allocation text and amounts. |

## Scoring and accounting

The terminal rationale uses these stable scoring IDs when explaining how the
public ledger, ordered pots, returns, and shares are awarded. Rust computes the
result and TypeScript only renders the supplied fields.

| Rule ID | Scoring/accounting rule | Applies to | Notes |
|---|---|---|---|
| `RL-SCORE-POT-AWARD` | Award the available public ledger to the last live seat when a foldout ends the hand, returning unmatched excess where applicable. | foldout terminal | Does not reveal folded seats' hole cards. |
...
| `RL-SCORE-SPLIT` | Divide each tied pot among that pot's tied eligible winners. | split showdown terminal | Canonical tied winners remain in stable seat/evaluation order. Remainder assignment follows `RL-POT-REMAINDER-001` and `RL-POT-ALLOC-002`. |
...
| `RL-SHOW-WINNER-001` | The best evaluated eligible hand wins each pot where exactly one eligible seat is strongest. | terminal | Rust computes and explains the decisive comparison. |
| `RL-SHOW-SPLIT-001` | Seats tied for best hand within a pot split that pot. | terminal | Equal shares are assigned first; tied winners are not re-ranked by payout order. |
...
River Ledger has foldout and showdown terminal result families. Showdown may
contain one pot or multiple ordered pots. Each terminal result may emit a
Rust-owned `terminal_rationale` with one of the template keys documented in
`UI.md`.
...
| `RL-END-SHOWDOWN` | The river betting round closes or all remaining seats are all-in, then Rust resolves one or more pots. | `river_ledger.showdown_best_hand_win` / `river_ledger.showdown_split_pot` | `best_showdown_hand` / `equal_best_hand_split` / per-pot eligibility |
...
| `RL-VIS-POT-001` | Starting stack, remaining stack, committed contribution, all-in status, current pot tiers, eligibility, and uncalled returns. | observer and all seat viewers | when public accounting is available | public view, seat view, WASM payload, DOM, replay export |
| `RL-VIS-POT-002` | Terminal per-pot awards and final allocation summaries. | observer and all seat viewers | terminal | Folded/non-showdown private cards stay redacted while public accounting remains visible. |
...
| `RL-REPLAY-STACK-001` | Stack, all-in, reopen, pot-tier, return, and per-pot allocation state is part of the River Ledger v2 replay/hash contract. | deterministic | viewer scoped | v1 internal replay is rejected rather than silently interpreted under v2 stack rules. |
...
| `RL-UI-POT-001` | Stack setup, all-in labels, pot tiers, eligibility, returns, and terminal allocations render Rust/WASM fields only. |
...
| `RL-BET-AMB-001` | Whether the v2 model can enter all-in states. | Yes; all-in is a stack-capped qualification on fixed-limit actions, not a new arbitrary-amount action family. | rule/property tests |
...
| `RL-VAR-ALLIN-001` | Many variants allow all-in and side pots. | River Ledger v2 includes fixed-limit all-in and ordered side-pot accounting inside `river_ledger_standard`. | Gate 15.1 scope is now implemented; no no-limit/pot-limit variant is added. |
...
## Gate 15 to Gate 15.1 migration rows

| Legacy rule ID | v2 treatment | Replacement rule IDs |
|---|---|---|
| `RL-BET-AMB-001` | Superseded. Base Gate 15 avoided all-in states; Gate 15.1 makes stack-capped all-in part of fixed-limit play. | `RL-STACK-SETUP-001`, `RL-ALLIN-CAP-001`, `RL-ALLIN-ACT-001` |
| `RL-POT-SINGLE-001` | Superseded as an exclusive term. A one-layer pot remains possible, but terminal accounting may include ordered side pots. | `RL-POT-LAYER-001`, `RL-POT-ALLOC-001`, `RL-OUTCOME-POTPRESENT-001` |
| `RL-POT-SINGLE-002` | Superseded. Contributions no longer necessarily feed one terminal pot. | `RL-POT-LAYER-001`, `RL-POT-LAYER-002`, `RL-POT-LAYER-003` |
| `RL-POT-ALLIN-001` | Superseded. All-in handling is implemented as stack-capped action metadata and pot eligibility. | `RL-ALLIN-CALL-001`, `RL-ALLIN-BET-001`, `RL-POT-RETURN-001` |
| `RL-VAR-ALLIN-001` | Superseded. All-in and side pots are now part of the standard v2 River Ledger variant. | `RL-ALLIN-REOPEN-001`, `RL-POT-ALLOC-002`, `RL-VIS-POT-001` |
| `RL-OOS-ALLIN-001` | Removed from out-of-scope status by Gate 15.1. The legacy ID remains only as a migration reference. | `RL-STACK-SETUP-002`, `RL-VIS-POT-002`, `RL-REPLAY-STACK-001`, `RL-UI-POT-001` |

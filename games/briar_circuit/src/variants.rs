...
use crate::ids::{
    ...
    STANDARD_HAND_SIZE, STANDARD_MAX_SEATS, STANDARD_MIN_SEATS, STANDARD_RANK_COUNT,
    STANDARD_SEAT_COUNT, STANDARD_SUIT_COUNT, STANDARD_TRICKS_PER_HAND, VARIANT_ID,
};
...
impl Manifest {
    pub fn parse(input: &str) -> Result<Self, String> {
        ...
        Ok(Self {
            ...
            pass_cycle: required_string(&values, "pass_cycle")?,
...
impl Variant {
    pub fn briar_circuit_standard() -> Self {
        Self {
            id: VARIANT_ID.to_owned(),
            display_name: "Briar Circuit".to_owned(),
            ...
            pass_cycle: "left_right_across_hold".to_owned(),
            deck_order: "standard_52_cards_clubs_diamonds_hearts_spades_rank_ascending_v1"
                .to_owned(),
            terminal_outcomes: "unique_low_score_win_threshold_tie_continues".to_owned(),
        }
    }

    pub fn resolve(id: &str) -> Result<Self, String> {
        match id {
            VARIANT_ID => Ok(Self::briar_circuit_standard()),
            _ => Err(format!("unsupported briar_circuit variant `{id}`")),
        }
    }
}
...
pub fn expected_manifest() -> Manifest {
    Manifest {
        ...
        display_name: "Briar Circuit".to_owned(),
        readiness: "admitted".to_owned(),
        rules_version: 1,
        rules_version_label: RULES_VERSION_LABEL.to_owned(),
        data_version: 1,
        data_version_label: DATA_VERSION_LABEL.to_owned(),
        schema_version: 1,
        theme_name: "briar_circuit_table".to_owned(),
        min_seats: STANDARD_MIN_SEATS,
        default_seats: STANDARD_DEFAULT_SEATS,
        max_seats: STANDARD_MAX_SEATS,
        supported_seats: "4".to_owned(),
        suit_count: STANDARD_SUIT_COUNT,
        rank_count: STANDARD_RANK_COUNT,
        card_count: STANDARD_CARD_COUNT,
        hand_size: STANDARD_HAND_SIZE,

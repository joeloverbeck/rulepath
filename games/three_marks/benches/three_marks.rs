...
use three_marks::{
    ...
    replay_support::{project_step, replay_commands},
    setup_match, validate_command, PublicView, SetupOptions, ThreeMarksLevel1Bot,
    ThreeMarksRandomBot, ThreeMarksState,
};

const RULES_VERSION: u32 = 1;
const GAME_ID: &str = "three_marks";
...
const THRESHOLDS: &[Threshold] = &[
    Threshold {
        ...
        caveat: "Initial floor below the first local native smoke measurement for the tiny fixed-board legal action tree.",
    },
    Threshold {
        ...
        threshold: 250_000.0,
        rationale_class: "measured_baseline",
        caveat: "Initial floor below the first local native full-report measurement for validation plus placement application.",
    },
    Threshold {
        ...
        threshold: 200_000.0,
        rationale_class: "measured_baseline",
        caveat: "Initial floor below the first local native full-report measurement for Rust-projected board view generation.",
    },
    Threshold {
        ...
        caveat: "Initial floor for board-aware replay step projection.",
    },
    Threshold {
        ...
        threshold: 25_000.0,
        rationale_class: "measured_baseline",
        caveat: "Initial floor below the first local native full-report measurement for public-view serialization round trips.",
    },
    Threshold {
        ...
        threshold: 25_000.0,
        rationale_class: "measured_baseline",
        caveat: "Initial floor below the first local native full-report measurement for deterministic five-command win replay.",
    },
    Threshold {
    ...
    Threshold {
        ...
        threshold: 150_000.0,
        rationale_class: "measured_baseline",
        caveat: "Initial floor below the first local native full-report measurement for seeded random legal selection from the starting action tree.",
    },
    Threshold {
        operation: "level1_bot_decision",
        unit: "decisions_per_second",
        threshold: 35_000.0,
        rationale_class: "measured_baseline",
        caveat: "Initial floor below the first local native full-report measurement for deterministic priority-policy selection from the starting board.",
    },
];
...
fn main() {
    ...
    println!("BEGIN_THREE_MARKS_BENCHMARK_JSON");
    println!("{}", benchmark_report_json(&metadata, &results));
    println!("END_THREE_MARKS_BENCHMARK_JSON");
}
...
fn run_benchmarks(filter: Option<&str>) -> Vec<BenchResult> {
    let benches: Vec<BenchSpec> = vec![
        ("legal_actions", "trees", 500_000, bench_legal_actions),
        ("apply_action", "actions", 500_000, bench_apply_action),
        (
...
fn print_human_summary(results: &[BenchResult]) {
    println!("three_marks native benchmarks");
...
fn benchmark_report_json(metadata: &ReportMetadata, results: &[BenchResult]) -> String {
    ...
    format!(
        concat!(
            ...
            "  \"schema_version\": {},\n",
            "  \"game_id\": \"{}\",\n",
            "  \"rules_version\": \"three_marks-rules-v{}\",\n",
            "  \"data_version\": \"{}\",\n",
            "  \"engine_version\": \"{}\",\n",
            "  \"build_profile\": \"{}\",\n",
            "  \"command\": \"{}\",\n",
...
fn bench_replay_step_projection(iterations: u64) {
    ...
    let command = command_for_state(&state, "place/r2c2");
    let action = validate_command(&state, &command).expect("benchmark command validates");
...
fn bench_serialization_roundtrip(iterations: u64) {
    let view = project_view(&initial_state(5), &Viewer { seat_id: None });
    let json = view.to_json();
    for _ in 0..iterations {
        let parsed = PublicView::from_json(black_box(&json)).expect("view parses");
...
fn run_random_playout(seed: u64) -> u64 {
    ...
    while state.terminal_outcome.is_none() {
        let bot = ThreeMarksRandomBot::new(Seed(seed.wrapping_add(actions)));
        let action_path = bot
...
fn initial_state(seed: u64) -> ThreeMarksState {
...
fn actor_for_state(state: &ThreeMarksState) -> Actor {
    Actor {
        seat_id: state.seats[state.active_seat.index()].clone(),
    }
}

fn command_for_state(state: &ThreeMarksState, segment: &str) -> CommandEnvelope {
    CommandEnvelope {
        actor: actor_for_state(state),
...
fn first_legal_segment(state: &ThreeMarksState) -> String {

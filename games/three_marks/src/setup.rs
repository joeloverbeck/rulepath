...
impl Default for SetupOptions {
    fn default() -> Self {
        Self {
            variant: Variant::three_marks_standard(),
        }
    }
}
...
) -> Result<ThreeMarksState, Diagnostic> {
    if options.variant.id != VARIANT_ID {
        return Err(Diagnostic {
            code: "unsupported_variant".to_owned(),
            message: "three_marks supports only three_marks_standard".to_owned(),
        });
    }

    let seat_count = options.variant.seat_count as usize;
    if SeatCountRange::inclusive(seat_count, seat_count)
        .expect("standard three_marks seat count is nonzero")
        .validate(seats.len())
        .is_err()
    {
        return Err(Diagnostic {
            code: "invalid_seat_count".to_owned(),
            message: "three_marks requires exactly two seats".to_owned(),
        });
    }

    Ok(ThreeMarksState {
        variant: options.variant.clone(),
        cells: ThreeMarksState::empty_cells(),
        active_seat: ThreeMarksSeat::from_index(options.variant.first_seat as usize).ok_or_else(
            || Diagnostic {
...
mod tests {
    ...
    #[test]
    fn setup_is_deterministic_empty_board_first_seat_non_terminal() {
        ...
        assert_eq!(left.active_seat, ThreeMarksSeat::Seat0);
    ...
    fn setup_rejects_wrong_seat_count() {
        ...
        assert_eq!(diagnostic.message, "three_marks requires exactly two seats");
    }
}

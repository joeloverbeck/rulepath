...
| `HCD-END-001` | Round six has resolved and terminal scoring runs. | `rules.rs`. | `terminal_after_six_rounds`; `terminal.trace.json`. | covered-by-trace | The browser is not the scoring authority. |
| `HCD-END-002` | One seat has a higher final score after the round limit and wins. | `rules.rs`, `visibility.rs`. | `terminal_winner_and_draw_policy`; `terminal.trace.json`. | covered-by-trace | Per-round explanation uses revealed history only. |
| `HCD-END-003` | Equal final scores after the round limit draw. | `rules.rs`, `visibility.rs`. | `terminal_winner_and_draw_policy`; draw full-trace replay seeds. | covered-by-trace | There is no tiebreaker. |
